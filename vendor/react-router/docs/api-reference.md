...
A comprehensive reference for the most commonly used APIs in React Router 7
framework mode, optimized for LLM consumption.
...
**Description:** Server-side data loading function that runs before route
components render. Only called on the server during SSR or build-time
pre-rendering.
...
**Description:** Server-side data mutation function called when forms are
submitted via `<Form>`, `useFetcher`, or `useSubmit`. Automatically revalidates
all loader data on the page.
...
**Description:** Client-side data loading that runs in the browser. Can
supplement or replace server loader data.
...
**Description:** Client-side data mutation that runs in the browser. Takes
priority over server actions when both are defined.
...
**Description:** Error boundary component that renders when route module APIs
throw errors.
...
**Description:** Progressively enhanced HTML form that submits data via fetch,
enabling advanced UIs with pending states and automatic revalidation.
...
**Description:** Progressively enhanced anchor tag for client-side navigation
...
function Navigation() {
  return (
    <nav>
      ...
      <Link to="/profile" state={{ from: 'navigation' }} viewTransition>
        Profile
      </Link>
    </nav>
  );
...
**Description:** Renders the matching child route component. Used in parent
routes to display nested routes.
...
**Description:** Returns the data from the closest route's loader or
...
**Description:** Returns the data from the last action submission, or undefined
if no action has been called.
...
**Description:** Returns a function for programmatic navigation. In framework
mode, returns a Promise.
...
**Description:** Provides components and utilities for data loading and
submission without navigation. Useful for complex, dynamic UIs.
...
**Description:** Returns information about pending page navigation, useful for
...
**Description:** Returns an object of key-value pairs of the dynamic params from
the current URL.
...
**Description:** Returns current URL search parameters and a function to update
them.
...
**Description:** Creates a redirect Response. Sets status code and Location
...
**Description:** Creates responses with custom headers/status without forcing
serialization into a Response object.
...
**Description:** Type guard to check if an error is a route error response (like
404, 500, etc.).
...
**Description:** Required root route that renders the HTML document and manages
...
**Description:** Required configuration file that maps URL patterns to route
modules.
...
**Description:** Framework mode provides automatic type generation for route
modules.
