...
immutable sequences of events. Each conversation is represented by a thread,
which provides a durable, resumable record of all interactions between users and
AI agents.
...
- **Working Conversation**: Current conversation state built from latest
  compaction + recent events
- **Complete History**: All events including compaction events (for
  debugging/audit)
...
- `buildWorkingConversation()` - Returns current conversation state
  (post-compaction)
...
The thread management system provides a robust foundation for conversational AI
with:
...
This design enables Lace to handle conversations of any length while maintaining
performance, reliability, and full conversation history. The elimination of
complex dual-ID systems and thread versioning makes the system much more
