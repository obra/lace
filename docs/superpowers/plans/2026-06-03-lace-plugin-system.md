...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. This is a large plan — execute it Part by Part. **Mandatory order: A → (B, C, D in any order) → E.** A is a hard prerequisite (nothing compiles without `plugins/`); E is last. B/C/D are independent once A lands. Default behavior is unchanged until a plugin is configured, EXCEPT Part C touches the live compaction call sites and Part E touches boot wiring — review those two with care. **Read the "Applied review corrections" section below first; it is authoritative over the body.**
...
## ⚠️ Coordination: the compaction registry is GREENFIELD (worktree has specs only)

Part C implements the compaction strategy registry **from scratch**. The `design/pluggable-compaction` worktree is in fact the PRI-2012 **spawn-broker branch** (its git log is all spawn-broker commits); it contains **no** compaction registry, `validatePreserved`, toolkit extraction, or `LACE_COMPACTION_PLUGINS` — only unimplemented design `.md` files. So there is **nothing to merge or supersede** and **zero collision risk**. Part C implements the worktree's Spec A §1 (seam + `validatePreserved` + toolkit) and §4 (route the 3 call sites) directly. The worktree's still-unwritten downstream work — breakpoints, the `compact_session` tool, the `sen-multiconv` regime (its Spec B) — will build on this plan's `api.compaction`. Authority: "lace leads the loader; compaction conforms" (Jesse, 2026-06-03).

---

## ✅ Applied review corrections (2026-06-03) — AUTHORITATIVE; override the body where they conflict

This plan was reviewed by roborev (codex) + a 3-opus adversarial panel. Verdict: the **architecture is sound** (subagent-reach-via-env verified end-to-end; the four-registry split; frame-buffering reasoning; Part A Phases 1–4 are clean and go green), but the body's code blocks in Parts B–E reference private/absent/invented symbols and missing test harnesses. The corrections below are **binding**; where a body code block conflicts, follow the correction. Jesse's three decisions are recorded inline.

### Blocking corrections
1. **Persona keystone — resolve at session construction, not a disk re-scan.** `RunnerConfig` has **no `personaName`** and `personaForSessionDir` is **module-private** in `storage/event-log.ts:61` AND a cold-cache disk scan — so the body's `this.config.personaName ?? personaForSessionDir(...)` won't compile and can ship `persona:''` in prod while green in CI. **Do instead:** (a) add `export` to `personaForSessionDir` (additive, explicit step); (b) add `persona?: string` to `RunnerConfig` (`core/conversation/types.ts:35-56`) and populate it where the runner is constructed (the persona is already known at `session/new`/`session/resume`; subagents pass it at `subagent-job.ts:968`); (c) the runner stamps `ToolContext.persona` from `this.config.persona` (drop the `personaForSessionDir` fallback for the security field). The Part B Phase 1 test must use a runner built with `config.persona='researcher'`, not a fake `sessionMeta` that the real code path never reads.
2. **Capability manifest — WIRE IT (Jesse's decision).** `Registry<T>` must track **owner**: `register(name, value, ownerPlugin)`, add `owner(name): string`. A plugin module **exports a `manifest`** (and its `meta`); the **loader** reads them and calls `recordManifest(meta.name, manifest)`; `createPluginApi` gives each plugin an owner-injecting registrar view (`api.tools.register(name, value)` records owner = that plugin's `meta.name`). Credential enforcement (#6) = `pluginMayUseCapability(registry.owner(toolName), 'credentials')`. Built-ins register with owner `'builtin'` (always allowed). The body's bare `register(name, value)` + uncalled `recordManifest` is replaced by this.
3. **Loader reads the plugin's declared `meta`** (not the raw specifier). `PluginModule` gains optional `meta` and `manifest` exports; the loader uses `mod.meta` for name/namespace/version and falls back to the specifier only if absent. Fixes the "namespacing-as-convention can't function" gap.
4. **Tools collision — register built-ins into `api.tools` (Jesse's decision; uniform dup→fatal).** Add `registerBuiltinTools(registries)` run in `boot()` **before** `loadPlugins`, registering every built-in tool (owner `'builtin'`) into `registries.tools`. A plugin tool with a built-in's name then hits **dup→fatal at load** (same model as compaction/runtimes), not silent shadowing. The per-session `ToolExecutor` draws **all** tools from `registries.tools` (built-ins now live there); option-taking built-ins (`DelegateTool` needs the persona registry, `UseSkillTool` needs the per-session skill registry) keep their per-session instantiation — register a name-reservation for those at boot so clashes still fatal, and instantiate the real one per session. Replaces Part B Phase 4's "built-ins win / silent skip."
5. **Part E — register built-in runtimes SYNCHRONOUSLY at module load; drop the boot-deferral.** Do **not** null out `containerManager` in `createAgentServerState`. Instead `registerBuiltinRuntimes()` runs at module load (before `createAgentServerState`), so `createDefaultContainerManager()` resolves from a populated registry synchronously and the manager stays non-null exactly as today. This avoids the `server.ts:357` `?.` silently dropping the network-lifecycle observer, the `PerInvocationReaper` `private readonly` mutator, and the "callers get null" regression. Embedder runtime plugins (the plane) are registered by the embedder before `createAgentServerState` runs (it sets `LACE_CONTAINER_RUNTIME=plane`); document that ordering requirement. (Runtime selection lives at `server.ts:100`, not `main.ts:17` — correct that anchor.)
6. **`validatePreserved` / `mergePreservedAdjacent` — write it; it does NOT exist to "promote."** `buildPreservedWithPrefix` (`track-compaction.ts:501`) only merges a prefix string into a leading user entry; it has no general same-role walk. `mergePreservedAdjacent` must be authored. **Spec it concretely:** operate on `preserved` as `Array<{role; content; toolCalls?; toolResults?}>` (the on-disk shape; element type is private — define a local `PreservedEntry`): (i) drop entries with empty/whitespace text content AND no toolCalls/toolResults; (ii) walk left→right, when entry[i].role === entry[i-1].role, merge: concat text content (array-concat `ContentBlock[]`, join strings with `\n`), union `toolCalls`, union `toolResults`; (iii) if the first entry is assistant-role, merge it forward into the next (or drop if it leaves nothing) so the result leads with user-role; (iv) return `[]` if nothing remains (caller → `{noop:true}`). Preserve image/resource `ContentBlock`s verbatim during merge. Add an **idempotency test** (`validatePreserved(validatePreserved(x)) === validatePreserved(x)`) and a **no-op-on-legacy test** (validatePreserved must not mutate today's `track-based` output — proves the seam is byte-safe). The C3 golden must **capture `compact()`'s output BEFORE the toolkit extraction** (add an explicit "Step 0: snapshot current output" step) — a post-refactor snapshot proves nothing.

### Required edits the body softened into asides (make them mandatory steps)
- **`tsx` devDependency (Jesse's decision).** Add `tsx` to `packages/agent` devDeps as an explicit step; the subagent-reach probe runs via `tsx`. ALSO make the reach test spawn the **real agent entrypoint** with `LACE_PLUGINS` set and assert a plugin-contributed persona/tool resolves in the child — the bespoke probe proves env+loader but not the production `argv[1]` path. Add a comment at `subagent-spawn.ts:51` that `...process.env` MUST inherit `LACE_PLUGINS` (reach breaks if anyone switches to a clean env).
- **Test isolation.** Add `resetRegistriesForTest()` to `plugins/api.ts` (mirroring `resetManifestsForTest`) and call it in `beforeEach` of every test that registers into the global `registries`; the `afterEach` "keep names unique" stubs are insufficient (dup→fatal within a file). Note the design relies on vitest per-file isolation.
- **`ToolContext.timeoutMs`** — add `timeoutMs?: number` to `ToolContext` (additive) as a committed step; the exec adapter reads it.
- **`ExecToolAdapter` advertised schema** — default `required: descriptor.inputSchema.required ?? []` (it's optional on the descriptor, required on `ToolInputSchema`). State explicitly that **lace performs no input validation for exec tools** (schema is `passthrough`); the binary is the validator.
- **Part B phase order** — move the descriptor (Phase 2a) before discovery, and `ExecToolAdapter`/`run-once` (Phase 3) before `discover.ts`, so each commit typechecks alone (`discover.ts` imports them).
- **Test harnesses don't exist** — `buildRunnerForTest`/`_runner-test-helpers`/`stubProviderEmittingToolCall` (Part B P1) and `_compaction-fixture`/`fixtureEvents`/`fixtureCtx` (Part C C3) are illustrative, not real. Either build the harness the way existing `runner.test.ts` does (hand-rolled `new ConversationRunner(config, deps)`), or assert at a smaller seam (unit-test a `resolvePersona`/strategy helper directly). Prefer the smaller seam.
- **MCP keystone gap** — until D2, MCP-adapter tools are **barred from the `credentials` capability** (default-deny already does this; state it). The keystone is delivered for in-process + exec adapters; MCP is the known gap.

### Decisions cut for simplicity
- **`PluginCallContext` is cut.** It was vestigial (no consumer; `signal`/`provider` are live objects, contradicting "marshalable"). Identity reaches tools via `ToolContext.persona` (the keystone). Reintroduce a marshalable context only when the out-of-process boundary (#6) needs it. Remove `plugins/context.ts` and its references from Part A Phase 2.
- **Compaction context** — add `sessionDir` to `CompactionContext` now (it's free; the runner has it) so the worktree's later `resolveModel`/`guidance` additions don't re-touch the three routed call sites for that field. Persona `compaction` schema block: this plan owns the full `{ strategy?, breakpoints? }` shape (declare both keys in the `.strict()` schema even though only `strategy` is read here) so the worktree's breakpoint work doesn't re-edit the same strict object.
- **Compaction bypass audit** — grep `import { compact }` / `from '@lace/agent/compaction/track-compaction'` across the tree; assert exactly the three known call sites route through the registry (no fourth bypass).
- **Strategy resolution timing** — resolve the persona's strategy name at **session open / persona load**, not first compaction, so a misconfigured persona fails at session start; ensure the resolve throw cannot abort a turn (keep it in the log-and-continue path).
...
- **Part A — Mechanism.** The loader, generic `Registry<T>` (owner-tracking — see corrections), `PluginApi` + `assertVersion`, the capability manifest (wired: plugin-exported manifest + owner-keyed default-deny credential gate), boot integration, and subagent reach. Establishes the four registries + the boot point where built-ins (incl. built-in tools) and plugins both register. (`PluginCallContext` is cut — see corrections.)
