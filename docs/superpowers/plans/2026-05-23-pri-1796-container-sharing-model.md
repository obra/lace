...
3. If absent, schedules one with cron `0 9 * * *` (daily at 09:00 in the agent's timezone) — the cron-at-least-5-min assertion (`assertCronAtLeast5MinInterval`) already passes this trivially.
4. If present, no-op.
...
- `packages/agent/src/tools/implementations/__tests__/delegate.test.ts`: assert background response shape includes `invocationId` and `scratchDir`. Assert sync mode prefix. Add tests GC-1 through GC-4 from Section 8 (scratch GC reminder lifecycle).
...
2. **Reminder cadence and time.** Plan picks daily at 09:00 in the agent's timezone. Could be twice-daily, weekly, or fire-on-disk-pressure. Implementer can adjust if there's a known cadence preference. The cron expression lives in `delegate.ts` (or the new `scratch-gc-reminder.ts` file) as a single constant — trivial to change after.
