...
- Needs **very explicit** instructions about architecture, code locations, and testing.
- Will benefit from strong guardrails: **DRY**, **YAGNI**, **TDD**, and **frequent commits**.

This plan assumes you will implement the work in small PRs with clean review points.
...
2. **Supervisor as the only web backend**
   ...
   - Supervisor spawns agent processes, routes prompts, streams updates, and handles approvals UI.

3. **Agent owns execution + persistence**
   - Tools run inside agent processes (local/remote future-proofing).
   - History is agent-owned append-only **JSONL**; browsing history requires live connectivity.

4. **Subagents exist as background jobs**
   - Agent can spawn subagent processes.
   - Supervisor can observe full job streams without forcing parent-context ingestion.
...
- Existing ThreadManager critique (historical context): `docs/plans/unexpose-thread-manager.md`
- Current architecture overview (historical context): `docs/plans/projects/projects-overview.md`
...
- **Protocol session**: The protocol’s `sessionId` (one agent conversation stream).
- **Supervisor**: Orchestrator process that spawns/monitors agent processes and serves the web UI.
- **Job**: Background execution unit emitted by an agent (`jobId`), including subagents and shells.
- **Durable events**: `ent/session/events` history (stable ordering via `eventSeq`).
- **Streaming updates**: `session/update` live updates (ordered via `streamSeq`).
...
- Supervisor process/library: `packages/supervisor` (binary optional: `lace-supervisor`)
...
Important: **Supervisor never executes tools**. All tool execution (filesystem, shell, network) happens in the agent process “world”.
...
1. **Write a failing test first** for every behavioral change (or update an existing failing test).
2. **Avoid mocks** of the behavior you are testing. Use small fakes only at process boundaries.
...
- Touch:
  - `packages/ent-protocol/package.json` (build/test scripts consistent with repo)
  - `packages/ent-protocol/tsconfig.json`
  - `packages/ent-protocol/src/index.ts`
  - Root `package.json` (workspace already includes `packages/*`, so no change needed)
- Tests:
  - Add vitest config only if needed (prefer repo defaults; do not invent new tooling).
- Acceptance:
...
- Implement:
  - TypeScript types and zod schemas for:
    ...
    - Core methods: `initialize`, `session/new`, `session/load`, `session/list`, `session/prompt`, `session/cancel`, `session/set_mode`
    - Ent extensions: `ent/agent/ping`, `ent/agent/status`, `ent/session/events`, `ent/session/configure`, `ent/session/inject`, `ent/job/*`, `ent/providers/*`, `ent/connections/*`, `ent/models/*`
  - Keep schemas DRY: shared “id fields”, shared “ContentBlock”, shared “ToolResult”.
- Touch:
...
- Tests:
  - `packages/agent/src/__tests__/smoke.test.ts` that spawns the server in-process using streams.
- Acceptance:
...
- Decide a path:
  - Use `getLaceDir()` from `packages/core/src/config/lace-dir.ts` (reuse, don’t rewrite).
  - Store sessions under something like: `<laceDir>/agent-sessions/<sessionId>/`.
- Touch:
...
- Implement:
  - A small `state.json` that stores:
    - last `eventSeq`
    - last `streamSeq` checkpoint (so stream ordering survives resume if we choose to persist it)
...
- Acceptance:
  - Restarting the agent process allows `ent/agent/status` to expose pending approvals.
...
Before integrating the full LLM/tools stack, build a minimal, deterministic loop:
- On `session/prompt`:
...
This is required so the supervisor/web can configure providers without owning credentials.
...
- `packages/agent/src/methods/ent-connections-test.ts` (YAGNI: implement minimal “can we instantiate provider + ping model list”; do not overbuild)

Tests:
- `packages/agent/src/methods/__tests__/ent-connections-crud.test.ts`

Acceptance:
- Supports multiple connections for the same provider with different base URLs and/or credentials.
...
- `packages/agent/src/methods/__tests__/ent-credentials-rotation.test.ts`
  - Rotating credentials for an existing `connectionId` replaces previous credentials.

Acceptance:
- “Assign new credentials to an existing endpoint” works (no forced new connectionId).
...
Acceptance:
- Lists models for a given connectionId; returns `providerId`, `connectionId`, `models[]`.
...
- Make `delegate` return a clear “not implemented” ToolResult, or remove the tool entirely (choose one; do not invent a new abstraction).
...
- Replace ThreadManager dependency with a minimal “event sink/source” owned by `@lace/agent`.
- Do not create a large new framework. Keep it to the smallest interface needed by `Agent`.
...
- Remove references to:
  - `packages/core/src/sessions/session.ts` (Agent should not know about multi-agent sessions)
...
Acceptance:
- `Agent` can run with an in-memory event store (for tests) and with JSONL store (in `@lace/agent`).
...
- `packages/agent/src/methods/session-request-permission.ts` (server-side plumbing)
...
- `packages/core/src/tools/implementations/*` (file read/edit/write, bash, search, url_fetch)

Touch:
- `packages/agent/src/runtime/tooling.ts` (construct ToolExecutor + register tools)
- `packages/agent/src/runtime/agent-runtime.ts` (construct core Agent with providers/tools)
...
- Shell tool execution should use core `bash` (do not add a second bespoke shell tool in `@lace/agent`).
...
- `packages/agent/src/streaming/update-emitter.ts` (to emit job_* + job_update)
...
- `packages/core/src/tools/implementations/delegate.ts` (or move delegate into agent package if that’s cleaner; pick one and keep it simple)
...
Acceptance:
- Supervisor can observe full subagent stream; parent doesn’t bloat its own context.
...
- Maintain a mapping: workspaceSession → sessionIds (grouping is supervisor-owned).
...
- Start supervisor + web, create a new agent session, send a prompt, see streaming text.
