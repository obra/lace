...
This document analyzes Lace's `session/update` protocol as defined in the ent-protocol package, documenting how session events flow from agent to supervisor to web clients, and what changes would be needed to add `turn_start`/`turn_end` as streaming events.
...
The `SessionUpdateNotificationSchema` defines the streaming events sent from agent to client during turn processing.
...
**`turn_start` and `turn_end` are NOT currently in the streaming protocol**. They exist only as:
...
The agent **writes** `turn_start`/`turn_end` as durable events but does **NOT** emit them via `emitSessionUpdate()`.
...
Note: Despite being handled as `onRequest`, this is semantically a notification (returns `undefined`).
...
The supervisor filters updates to match the expected session and forwards to configured handlers.
...
> **Durable event guarantee**: A successful `session/prompt` response implies that corresponding durable events (`turn_start`, `message`, `tool_use`, `turn_end`) have been written and can be fetched via `ent/session/events` with stable ordering.

The current design treats `turn_start`/`turn_end` as **durable markers** rather than streaming events. Clients can:
...
```typescript
await writeAndAdvance({ type: 'turn_end', data: { stopReason } });
await emitSessionUpdate({ type: 'turn_end', turnId, stopReason }, { turnId, turnSeq: finalTurnSeq });
```
...
However, this appears to be **reading from the fake agent fixtures** or **anticipating future protocol support**, as the real agent doesn't emit these streaming events yet.
...
- `turn_start`/`turn_end` exist only as **durable** events (written to events.jsonl)
...
The TUI codebase already anticipates these events, suggesting this was planned but not yet implemented.
