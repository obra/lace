...
**Context:** We have imported a comprehensive atomic design system into the Lace
AI assistant codebase. Phase 1 (chat input upgrade) is complete. This document
covers migrating the remaining UI components to use the design system.
...
**Status:** ⚠️ **SUPERSEDED BY NEW PLAN** - See
`docs/plans/2025-07-21/phase2-timeline-migration.md`

**Previous Goal:** Replace custom message rendering with design system
components.

**New Approach:** Use proper TimelineView components + API cleanup for full
design system compliance.

**Current State:** Custom LaceMessageList implementation completed but needs to
be replaced with proper TimelineView integration.

### Task 2.1: Analyze Current Message Rendering

**Files to examine:**

- `packages/web/components/old/ConversationDisplay.tsx` - Current message
  display logic
...
1. **Event types** - USER_MESSAGE, AGENT_MESSAGE, TOOL_CALL, TOOL_RESULT,
   AGENT_TOKEN
...
**File to create:**
`packages/web/components/ui/__tests__/LaceMessageDisplay.test.tsx`
...
**File to create:**
`packages/web/components/ui/__tests__/LaceMessageList.test.tsx`
...
**Commit:**
`feat: replace ConversationDisplay with design system message components`
...
- Available design system components in Storybook:
  ...
  - Navigate to
    `http://localhost:6006/?path=/docs/organisms-mobilesidebar--docs`
...
**File to create:**
`packages/web/components/ui/__tests__/LaceSidebarContent.test.tsx`
...
**File to create:**
`packages/web/components/ui/__tests__/LaceAppLayout.test.tsx`
...
**File to create:**
`packages/web/components/ui/__tests__/LaceToolApprovalModal.test.tsx`
...
<AnimatedModal isOpen={true} onClose={() => {}}>
  <div className="p-6">
    ...
    <div className="timer">{timeRemaining}s remaining</div>

    <div className="actions">
      <AnimatedButton onClick={() => onDecision('APPROVE')}>
        Approve
      </AnimatedButton>
      <AnimatedButton onClick={() => onDecision('DENY')}>Deny</AnimatedButton>
    </div>
  </div>
</AnimatedModal>
```
...
**File to create:**
`packages/web/components/ui/LaceToolApprovalModal.stories.tsx`
...
**Types:** `feat`, `fix`, `test`, `docs`, `refactor` **Scopes:** `ui`, `layout`,
`modal`, `message`
