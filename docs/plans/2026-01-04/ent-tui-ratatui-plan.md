...
Jesse: this doc is written for a skilled engineer with **almost zero context** on Lace/Ent, uneven taste, and limited test-design instincts. It is intentionally explicit and procedural.

## One-Sentence Summary

Build a **premium-feeling terminal UI** (Rust + `ratatui`) that drives **any Ent-protocol-speaking agent** over **NDJSON JSON-RPC over stdio**, with great streaming, a first-class permission UI, and toggleable panes.

This lives in a new package **next to** `packages/cli` (the current line-based REPL), and does **not** replace it.

## Hard Constraints (Do Not Violate)

- **DRY / YAGNI:** build the minimum that feels excellent; no speculative frameworks.
- **TDD:** write tests first for any non-trivial logic (reducers/state machines/parsers).
- **Frequent commits:** commit after each bite-sized task once tests pass.
- **Compatibility:** must work with **non-Lace** agents implementing Ent protocol (no Lace-only assumptions).
- **Permissions:** always show tool input (or `<unavailable>` if missing); no “edit tool input” UX for v1.
- **Panes are toggleable:** user can show/hide Chat, Activity, Logs/Debug (and later Files/Diff).

## Read This First (Context You Need)

### Protocol & baseline CLI

- `docs/plans/2026-01-03/ent-cli-client-spec.md` (baseline expectations, method names, prompts, config behavior).
...
- `docs/plans/tool-renderers.md` + `docs/plans/toolbox-style.md` (how to present tool calls cleanly).
...
- Right: **activity timeline** (tool calls, jobs, key events, errors) as compact cards.
...
This replaces a cluttered keybinding matrix and is the main discoverability feature.
...
Permission requests arrive via `session/request_permission` (request; must respond).

Modal must show:
- tool name, kind, resource
- `toolCallId`, `turnId`, `turnSeq`, `jobId` if present
- **tool input**:
  - if we’ve seen a matching `tool_use` update with the same `toolCallId`, show its JSON
...
- Async: start with **threads + channels** (std) unless we hit pain. Tokio is optional; don’t default to it.
- Process spawn: `std::process::Command`
- Testing: built-in `cargo test` + snapshot testing only if it truly reduces boilerplate (avoid golden snapshots for volatile UI).
...
Do not create a top-level Cargo workspace at repo root unless Jesse explicitly requests it.
...
Use a fake agent that speaks JSON-RPC over stdio (NDJSON). Reuse the existing Node fixtures to avoid rewriting protocol fakes unless necessary:
...
- Build & run:
  ...
  - or: `cargo run -- --workdir "$(pwd)" --agent-cmd "node ../../packages/agent/dist/main.js"`
- Unit tests:
  - `cd packages/tui && cargo test`
- E2E tests (spawns Node fake agent):
  - `cd packages/tui && cargo test --test e2e_*`

If Node isn’t on PATH, install it (repo already requires Node for other packages).
...
**Goal:** parse `--agent-cmd`, `--workdir`, `--load`, `--new`, `--timeout-ms` (optional).

- Files:
  - `packages/tui/src/args.rs`
  - update `packages/tui/src/main.rs`
- Tests (first):
  - `packages/tui/src/args.rs` unit tests: mutual exclusion (`--load` + `--new`), defaults, workdir handling
...
**Goal:** on startup, send `initialize`, then `session/new` (default) or `session/load`.
...
**Goal:** show user + assistant messages; stream `text_delta` inline; finalize on `turn_end`.
...
- Tests (first):
  ...
  - fallback: prompt response with `stopReason` finalizes if `turn_end` never arrives
...
**Goal:** make Chat/Activity/Debug panes toggleable; Tab cycles focus; input focus is stable.
...
**Goal:** `Ctrl+K` palette with a few actions; fuzzy search is optional (start with substring).
...
- Files:
  ...
  - update `docs/plans/2026-01-04/ent-tui-ratatui-plan.md` with “what changed” notes if needed
- Checklist:
...
- Don’t assume Lace-only RPC methods exist (e.g. `ent/*`); gate them behind “method exists” checks if we add them later.
