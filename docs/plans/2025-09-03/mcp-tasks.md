...
Lace is a TypeScript/Node.js AI coding assistant using event-sourcing
architecture. All conversations are stored as immutable event sequences. The
tool system uses Zod schemas for validation and extends a base `Tool` class.

### Key Architectural Patterns

- **Test-Driven Development (TDD)**: Write failing tests first, implement
...
- **Process Isolation**: External services (like MCP servers) run in separate
  processes
...
- `packages/core/src/tools/implementations/bash.ts` - Example tool
  implementation
- `vendor/typescript-sdk/src/client/index.ts` - MCP Client API
- `vendor/typescript-sdk/src/client/stdio.ts` - Stdio transport for process
  spawning
...
The MCP TypeScript SDK provides high-level abstractions that handle all JSON-RPC
communication. **DO NOT implement custom JSON-RPC handling.** Instead use:
...
- **Integration Tests**: Test component interactions (e.g., tool registration +
  execution)
- **E2E Tests**: Test complete workflows (config loading → server startup → tool
  execution)
...
**Objective**: Define TypeScript interfaces for MCP configuration and state
management
...
**Commit Message**:
`feat: add MCP types and interfaces for server configuration`

---

#### Task 1.2: Create Configuration Loader

**Objective**: Load and merge MCP configuration files with hierarchy (global →
project)
...
- Look at existing config loading patterns in `packages/core/src/config/`
  directory
...
const MCPServerConfigSchema = z
  ...
  .refine((config) => config.transport !== 'tcp' || config.port !== undefined, {
...
describe('MCPConfigLoader', () => {
  ...
  it('should require port for TCP transport', () => {
    ...
    expect(() => MCPConfigLoader.loadConfig(tempDir)).toThrow(
      'TCP transport requires port'
    );
  });
  ...
  it('should merge configs with project replacing global servers', () => {
    ...
    writeFileSync(
      join(globalConfigPath, 'mcp-config.json'),
      JSON.stringify({
        servers: {
          filesystem: {
    ...
    writeFileSync(
      join(projectConfigPath, 'mcp-config.json'),
      JSON.stringify({
        servers: {
          filesystem: {
    ...
    const config = MCPConfigLoader.loadConfig(tempDir);
    expect(config.servers.filesystem.command).toEqual([
      'node',
      'project-fs.js',
    ]);
...
**Commit Message**:
`feat: add MCP configuration loader with hierarchical merging`

---

#### Task 1.3: Create MCP Tool Adapter

**Objective**: Create adapter that wraps MCP tools to work with existing Tool
base class
...
- Look at `packages/core/src/tools/implementations/bash.ts` for example
  implementation
  ...
  private async startTcpServer(connection: MCPServerConnection): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      socket.on('error', (error) => {
        reject(
          new Error(
            `Failed to connect to TCP server on port ${port}: ${error.message}`
          )
        );
      });
    });
...
describe('MCPServerManager', () => {
  ...
  it('should validate TCP configuration', async () => {
    ...
    await expect(manager.startServer('tcp-server', config)).rejects.toThrow(
      'TCP transport requires port'
    );
  });
...
**Commit Message**:
`feat: add MCP server manager with process lifecycle support`

---

#### Task 2.2: Create MCP Tool Registry

**Objective**: Discover tools from MCP servers and present them through unified
interface
...
export class MCPToolRegistry extends EventEmitter {
  ...
  async initialize(config: MCPConfig): Promise<void> {
    ...
    const startPromises = Object.entries(config.servers)
      ...
      .map(([serverId, serverConfig]) =>
        this.serverManager
          .startServer(serverId, serverConfig)
          .catch((error) => {
  ...
  private async discoverServerTools(serverId: string): Promise<void> {
    ...
    try {
      ...
      const adaptedTools = tools.map(
        (mcpTool) =>
          new MCPToolAdapter(mcpTool, serverId, (request) =>
            this.serverManager.sendRequest(server, request)
          )
      );
  ...
  private async listServerTools(
    ...
    if (response.error) {
      throw new Error(
        `Failed to list tools from server ${server.id}: ${response.error.message}`
      );
    }
...
describe('MCPToolRegistry', () => {
  ...
  it('should get correct approval level for tools', () => {
    ...
    expect(registry.getToolApprovalLevel(config, 'filesystem/read_file')).toBe(
      'allow-session'
    );
    expect(registry.getToolApprovalLevel(config, 'filesystem/write_file')).toBe(
      'require-approval'
    );
    expect(
      registry.getToolApprovalLevel(config, 'filesystem/unknown_tool')
  ...
  it('should handle tool discovery errors gracefully', async () => {
    ...
    vi.spyOn(mockServerManager, 'sendRequest').mockRejectedValue(
...
**Commit Message**:
`feat: extend approval system with project and always approval levels`

---

#### Task 3.2: Integrate MCP with ToolExecutor

**Objective**: Register MCP tools with existing ToolExecutor and handle approval
workflow
  ...
  private async initializeMCPRegistry(): Promise<void> {
    try {
      const config = MCPConfigLoader.loadConfig(
        this.envManager.getProjectRoot()
      );
...
**Commit Message**:
`feat: integrate MCP tools with ToolExecutor and approval system`
...
const CreateServerSchema = z
  ...
  .refine((data) => data.transport !== 'tcp' || data.port !== undefined, {
    message: 'TCP transport requires port to be specified',
  });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const projectRoot =
      request.nextUrl.searchParams.get('projectRoot') || undefined;
...
describe('/api/mcp/servers', () => {
  ...
  describe('GET', () => {
    it('should return server list with status', async () => {
      ...
      vi.mocked(MCPServerManager).mockImplementation(
...
**Commit Message**:
`feat: add MCP server list API routes with validation and testing`

---

#### Task 4.2: Create Server Control API Routes

**Objective**: Create API routes for individual server management (status,
control, tools)
...
**Test to Write
(`packages/web/app/api/mcp/servers/[serverId]/control/route.test.ts`):**
...
describe('/api/mcp/servers/[serverId]/control', () => {
  ...
  it('should handle restart action', async () => {
    ...
    const mockServerManager = {
      ...
      getServer: vi
        .fn()
        .mockReturnValue({ id: 'filesystem', status: 'running' }),
    };
  ...
  it('should return 404 for non-existent servers', async () => {
    ...
    const request = new NextRequest(
      'http://localhost/api/mcp/servers/nonexistent/control',
      {
        method: 'POST',
        body: JSON.stringify({ action: 'start' }),
      }
...
describe('MCP Integration E2E', () => {
  ...
  it('should handle server failures gracefully', async () => {
    ...
    const availableTools = toolRegistry.getAvailableTools(
      MCPConfigLoader.loadConfig(tempDir)
    );
  ...
  it('should respect tool approval policies', async () => {
    ...
    writeFileSync(
      join(laceDir, 'mcp-config.json'),
      JSON.stringify(configWithDisabledTool)
    );
...
Lace supports external MCP servers to extend tool capabilities beyond the
built-in tools.
...
3. **Start Lace** - MCP servers will start automatically and their tools will be
   available to AI providers.
...
Access MCP management through the web interface at
...
Create `{project}/.lace/mcp-config.json` to override global settings for
specific projects:
...
**Important**: Project servers completely replace global servers with the same
name. There is no inheritance of tool policies.
...
**Commit Message**:
`feat: complete MCP integration with full test coverage and documentation`

---

## Summary

This implementation plan provides a complete, production-ready MCP integration
for Lace that:

✅ **Maintains Clean Architecture**: Uses existing patterns and doesn't disrupt
current functionality ✅ **Provides Comprehensive Testing**: Unit, integration,
and E2E tests with good coverage  
✅ **Follows TDD Principles**: Tests written first, minimal implementation to
pass ✅ **Includes Full Documentation**: User guides, API docs, and inline code
documentation ✅ **Handles Edge Cases**: Server failures, network issues,
configuration errors ✅ **Supports All Requirements**: Process isolation,
flexible transports, granular approval policies

The plan is broken into bite-sized tasks with clear objectives, required files,
implementation details, and testing requirements. Each task builds on previous
work and can be committed independently.

**Estimated Implementation Time**: 15-20 days for experienced developer **Key
Dependencies**: @modelcontextprotocol/sdk, existing Lace tool system **Testing
Strategy**: 70+ test cases covering happy path, edge cases, and error conditions
