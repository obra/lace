...
communication. **DO NOT implement custom JSON-RPC handling.** The previous
implementation plan was incorrect. Instead use:
...
describe('MCPConfigLoader', () => {
  ...
  it('should merge configs with project replacing global servers', () => {
    ...
    const projectConfig = {
      servers: {
        fs: { command: 'project', enabled: false, tools: { read: 'deny' } },
      },
    };
...
describe('MCPServerManager', () => {
  ...
  it('should create client and transport instances', async () => {
    const { Client } = await import(
      '../../vendor/typescript-sdk/src/client/index.js'
    );
    ...
    for (const [key, prop] of Object.entries(schema.properties || {})) {
      ...
      if (propSchema.type === 'string') {
        ...
        const itemType = propSchema.items
          ? jsonSchemaToZod(propSchema.items)
          : z.unknown();
...
describe('MCPToolAdapter', () => {
  ...
  it('should execute MCP tool and return success result', async () => {
    const mockCallTool = mockClient.callTool as vi.MockedFunction<
      typeof mockClient.callTool
    >;
...
**Commit Message**:
`feat: extend approval system with project/always/disable levels`
...
export class ToolExecutor {
  ...
  async executeTool(
    ...
    if (mcpApprovalLevel) {
      ...
      if (
        mcpApprovalLevel === 'allow_always' ||
        mcpApprovalLevel === 'allow_project'
      ) {
...
**Commit Message**:
`feat: integrate MCP tools with ToolExecutor approval system`
    ...
    vi.mocked(Client).mockImplementation(
      () =>
        ({
          connect: vi
            .fn()
            .mockRejectedValue(new Error('ENOENT: command not found')),
          close: vi.fn(),
        }) as any
    );
...
**Commit Message**:
`test: add comprehensive MCP integration test using SDK mocks`
...
**Objective**: Ensure project dependencies are correctly configured for MCP
integration
...
5. **Documentation update:** Update README with corrected information about SDK
   usage.

**Commit Message**:
`feat: complete MCP integration using official TypeScript SDK with full test coverage`

---

## Summary

This revised implementation plan provides a **much simpler and correct** MCP
integration that:

✅ **Uses Official SDK**: Leverages high-level `Client` and
`StdioClientTransport` classes ✅ **No Custom JSON-RPC**: Eliminates unnecessary
protocol implementation ✅ **Maintains Clean Architecture**: Integrates with
existing Tool system seamlessly ✅ **Comprehensive Testing**: Unit, integration,
and E2E tests with SDK mocking ✅ **TDD Approach**: Tests written first with
clear failure/success criteria
...
**Estimated Implementation Time**: 8-10 days for experienced developer **Key
Dependencies**: Official MCP TypeScript SDK (vendored) **Testing Strategy**: 40+
test cases with SDK mocking for reliable, fast tests
