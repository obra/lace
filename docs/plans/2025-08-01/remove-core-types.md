...
- Delete `TaskFilters` interface from `packages/web/lib/client/task-api.ts`
  (lines 6-11)
- Update import in `packages/web/lib/client/task-api.ts` to include
  `TaskFilters` from `@/types/core`

## Step 3: Update hooks to use core TaskFilters

- Update import in `packages/web/hooks/useTaskManager.ts` from
  `@/lib/client/task-api` to include `TaskFilters` from `@/types/core`
