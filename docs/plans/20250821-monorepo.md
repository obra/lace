...
Lace started as a simple CLI app, then added a web interface as a package, then
removed the CLI leaving the backend at the top level. This created a weird
half-monorepo setup with:
...
3. Missed hardcoded `src/` path assumptions in `resource-resolver.ts` and other
   files
...
**Key insight:** There are hardcoded path assumptions throughout the codebase
that only surface during runtime or complex operations.
...
1. **Update resource-resolver.ts first** (most critical)
   ...
   // To:
   const srcIndex =
     moduleDir.indexOf('/packages/core/src/') !== -1
...
6. **Use facades properly** - Web package should only import through controlled
   interfaces
...
❌ **Complex conflict resolution**: Too many changes make conflicts hard to
resolve  
❌ **Direct core imports**: Web package should use facades, not `@lace/core`
directly
...
The incremental approach with proper git moves and constant testing is the only
reliable way to do this refactor safely.
