...
This document covers the remaining integration work needed to complete the
OpenRouter dynamic catalog feature after Phase 6 implementation.
...
test('OpenRouter full flow', async ({ page }) => {
  ...
  await page.reload();
  await expect(
    page.locator('[aria-label="OpenAI provider toggle"]')
  ).not.toBeChecked();
});
```
...
2. **API Rate Limits**: OpenRouter may rate limit - implement exponential
   backoff
