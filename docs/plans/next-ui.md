...
1. **Provider Discovery API** - GET /api/providers endpoint returns all
...
4. **Type System Improvements** - Proper separation of UI and backend event
   types
...
1. **Created BaseMockProvider** - Base class for test providers that implements
...
Implement a clean Next.js-based web UI for Lace using test-driven development.
The goal is to create a shared SSE stream architecture that supports the
existing single-agent workflow and future multi-agent capabilities.
...
- **Agents**: Individual AI instances (maps to child threads like
  `lace_20250703_abc123.1`)
- **Event Sourcing**: All state changes flow through immutable events
- **Session-Scoped Streams**: One SSE stream per session for focused event
  delivery
...
- **Sessions = Parent Threads**: Sessions are threads with `{ isSession: true }`
  metadata
- **Agent Access Pattern**: ThreadManager accessed exclusively through Agent
  instances
...
**File**:
`src/interfaces/web/app/api/sessions/[sessionId]/agents/__tests__/route.test.ts`
...
**File**:
`src/interfaces/web/app/api/threads/[threadId]/message/__tests__/route.test.ts`
...
**File**:
`src/interfaces/web/app/api/sessions/[sessionId]/events/stream/__tests__/route.test.ts`
...
```typescript
class SSEManager {
  private sessionStreams = new Map<
    string,
...
**Session Management**: `src/interfaces/web/app/api/sessions/route.ts` **Agent
Spawning**: `src/interfaces/web/app/api/sessions/[sessionId]/agents/route.ts`
**Message Sending**:
`src/interfaces/web/app/api/threads/[threadId]/message/route.ts` **SSE Stream**:
`src/interfaces/web/app/api/sessions/[sessionId]/events/stream/route.ts`
...
- `src/interfaces/web/app/api/sessions/__tests__/route.test.ts` - Session
  management
- `src/interfaces/web/app/api/sessions/[sessionId]/agents/__tests__/route.test.ts` -
  Agent spawning
- `src/interfaces/web/app/api/threads/[threadId]/message/__tests__/route.test.ts` -
...
- `src/interfaces/web/app/api/sessions/[sessionId]/events/stream/__tests__/route.test.ts` -
  SSE streaming
- `src/interfaces/web/__tests__/integration/full-flow.test.ts` - Full
  conversation flow
...
- `packages/web/app/api/sessions/[sessionId]/events/stream/route.ts` - SSE
  streaming
...
- `packages/web/app/page.tsx` - Full test UI with session management, agent
  spawning, and messaging
- `packages/web/components/ConversationDisplay.tsx` - Terminal-like conversation
  display component

**Implementation Notes:**

- Fixed Next.js 15 async params requirement
- Webpack warnings about dynamic imports are expected and safe
  (ProviderRegistry)
- Full UI with real-time event streaming working
- Session names and metadata stored in memory only (needs backend update for
  persistence)
...
**GET /api/providers** Returns all available providers and their models in a
single call.
...
**POST /api/providers/{provider}/configure** (Future) Configure credentials for
a specific provider.
...
**GET /api/providers/{provider}/test** Test if a provider is properly configured
and working.
...
2. **Provider Base Class Enhancement:** ✅
   - Added abstract methods: `getProviderInfo()`, `getAvailableModels()`,
     `isConfigured()`
...
This plan provides a clear path from test-driven API development to a functional
web UI that proves the architecture works for both single-agent and future
