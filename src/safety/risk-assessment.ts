...
function assessShellRisk(input: Record<string, any>): RiskLevel {
  ...
  const dangerousPatterns = [
    ...
    'curl', 'wget', 'git clone', 'npm install -g',
...
function assessFileRisk(toolName: string, input: Record<string, any>): RiskLevel {
  // Check if this is explicitly a read-only operation
  const isReadOnly = toolName.includes('read') && !toolName.includes('write') && !toolName.includes('edit');
  if (isReadOnly) {
    return 'low';
  }

  // Check if this involves writing/editing files
  const isWriteOperation = toolName.includes('write') || toolName.includes('edit') || 
  ...
  if (isWriteOperation) {
    ...
    const sensitivePatterns = [
      '/etc/', 'package.json', '.env', 'config',
      'passwd', 'shadow', 'hosts', '.ssh',
      'authorized_keys', 'dockerfile', 'docker-compose'
    ];
...
function isJavaScriptTool(toolName: string): boolean {
  return toolName.includes('javascript') || 
...
function assessJavaScriptRisk(input: Record<string, any>): RiskLevel {
  ...
  const dangerousPatterns = [
    'require', 'import', 'process', 'eval',
    'exec', 'spawn', 'fs.', 'filesystem',
    'unlinksync', 'rmdirsync', 'exit',
