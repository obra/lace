...
vi.mock('../../../ui/TimelineEntryCollapsibleBox.js', () => ({
  TimelineEntryCollapsibleBox: ({ children, summary, isExpanded, label }: any) => {
    ...
    const contentText = isExpanded ? (typeof children === 'object' ? '[ComplexContent]' : children) : '';
...
describe('GenericToolRenderer', () => {
  ...
  describe('Content rendering', () => {
    ...
    it('should show input and output when expanded', () => {
      const item = createToolExecutionItem('bash', { command: 'ls' }, createSuccessResult('file.txt'));
      
      const { lastFrame } = render(
        <GenericToolRenderer item={item} isExpanded={true} />
      );
    ...
    it('should not show compact output preview for errors when collapsed', () => {
      const item = createToolExecutionItem('bash', { command: 'ls' }, createErrorResult('Permission denied'));
  ...
  describe('JSON output detection', () => {
    it('should detect JSON output and set appropriate language', () => {
      ...
      const item = createToolExecutionItem('bash', { command: 'curl' }, createSuccessResult(jsonOutput));
    ...
    it('should default to text for non-JSON output', () => {
      ...
      const item = createToolExecutionItem('bash', { command: 'ls' }, createSuccessResult(textOutput));
  ...
  describe('Error handling', () => {
    it('should display error message when tool fails', () => {
      const item = createToolExecutionItem('bash', { command: 'invalid' }, createErrorResult('Command not found'));
