...
export function useTextBuffer(initialText: string = ''): [TextBufferState, TextBufferOperations] {
  ...
  const [debugLog, setDebugLog] = useState<string[]>(['TextBuffer initialized']);
  ...
  const insertText = useCallback((text: string) => {
    setDebugLog(prev => [...prev.slice(-4), `INSERT: "${text}" at pos ${cursorColumn}`]);
  ...
  const deleteChar = useCallback((direction: 'forward' | 'backward') => {
    if (direction === 'backward' && cursorColumn > 0) {
      ...
      setDebugLog(prev => [...prev.slice(-4), `BEFORE: line="${currentLine}" pos=${cursorColumn} deleting="${charToDelete}"`]);
      ...
      setDebugLog(prev => [...prev.slice(-4), `AFTER: newPos=${cursorColumn - 1}`]);
  ...
  const moveCursor = useCallback((direction: 'left' | 'right' | 'up' | 'down' | 'home' | 'end') => {
    ...
    switch (direction) {
      ...
      case 'up':
        if (cursorLine > 0) {
          setCursorLine(cursorLine - 1);
          setCursorColumn(Math.min(cursorColumn, lines[cursorLine - 1].length));
        }
        break;
      case 'down':
