...
export class RestoreOperations {
  constructor(snapshotManager, gitOperations, projectPath) {
    ...
    this.restoreHistoryPath = join(projectPath, '.lace', 'restore-history.json');
  }
  ...
  async performSafetyCheck() {
    ...
    if (hasChanges) {
      recommendations.push('Commit or stash your current changes before restoring');
      recommendations.push('Consider creating a backup branch of your current state');
      recommendations.push('Use --force flag only if you are certain about discarding changes');
    }
  ...
  async restoreFromSnapshot(snapshotId, options = {}) {
    ...
    if (!options.force) {
      ...
      if (!safetyCheck.safe) {
        throw new Error('Working tree has uncommitted changes. Use { force: true } to override or commit/stash changes first.');
      }
    }
  ...
  async restoreFiles(snapshotId, filePaths) {
    ...
    try {
      const restoreResult = await this.gitOps.restoreFiles(snapshot.gitCommitSha, filePaths);
  ...
  async recordRestoration(operation) {
    ...
    await fs.writeFile(this.restoreHistoryPath, JSON.stringify(history, null, 2));
  }
    ...
    if (targetSnapshot.toolCall && targetSnapshot.toolCall.executionId) {
      ...
      for (const snapshot of allSnapshots) {
        if (snapshot.snapshotId !== snapshotId && 
          ...
          const fullSnapshot = await this.getSnapshotDetails(snapshot.snapshotId);
