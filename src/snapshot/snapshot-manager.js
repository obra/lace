...
export class SnapshotManager {
  ...
  async initialize() {
    try {
      ...
      throw new Error(
        `Failed to initialize snapshot manager: ${error.message}`,
      );
    }
  }
    ...
    try {
      ...
      if (this.contextCapture && sessionId !== null && generation !== null) {
        try {
          ...
          enrichedContext = {
            ...legacyContext,
            contextCaptureError: error.message,
          };
        }
      }
      ...
      const metadata = {
        ...
        gitCommitSha,
        realRepoSha:
          enrichedContext.realRepoSha || (await this.getRealRepoSha()),
        toolCall,
    ...
    if (filters.tool) {
      ...
      for (const snapshot of snapshots) {
        try {
          ...
          if (
            metadata.toolCall &&
            metadata.toolCall.toolName === filters.tool
          ) {
  ...
  async applyRetentionPolicy() {
    const index = await this.getSnapshotIndex();
    const { maxSnapshots, maxAge, keepCheckpoints } =
    ...
    if (index.snapshots.length > maxSnapshots) {
      ...
      toDelete.push(
        ...excess.filter((s) => !keepCheckpoints || s.type !== "checkpoint"),
      );
    }
    ...
    if (maxAge) {
      ...
      const expired = index.snapshots.filter((s) => {
        ...
        return (
          ...
          (!keepCheckpoints || s.type !== "checkpoint")
        );
      });
