...
export class GitOperations {
    ...
    try {
      ...
      try {
        ...
        if (!stats.isDirectory()) {
          throw new Error(
            `Project path is not a directory: ${this.projectPath}`,
          );
        }
      } catch (error) {
      ...
      try {
        ...
        console.warn(
          "Git config setup failed (may be due to test cleanup):",
          configError.message,
        );
      }
    } catch (error) {
  ...
  async checkout(commitSha) {
    try {
      ...
      throw new Error(
        `Failed to checkout commit ${commitSha}: ${error.message}`,
      );
    }
  }
  ...
  async getDiffFiles(fromCommit, toCommit) {
    try {
      ...
      return {
        modified: diffSummary.files
          .filter((f) => f.changes > 0 && !f.binary)
          .map((f) => f.file),
        added: diffSummary.files
          .filter((f) => f.insertions > 0 && f.deletions === 0)
          .map((f) => f.file),
      ...
      throw new Error(
        `Failed to get diff between ${fromCommit} and ${toCommit}: ${error.message}`,
      );
    }
  }
  ...
  async restoreFiles(commitSha, filePaths) {
    try {
      ...
      throw new Error(
        `Failed to restore files from ${commitSha}: ${error.message}`,
      );
    }
  }
