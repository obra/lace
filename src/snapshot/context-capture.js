...
export class ContextCapture {
  ...
  async captureConversationContext(sessionId, generation) {
    ...
    try {
      ...
      const limitedHistory = recentHistory
        ? recentHistory.slice(0, this.config.conversationTurnsToCapture)
        : [];
  ...
  async captureFullContext(sessionId, generation, toolCall, projectPath) {
    try {
      ...
      const toolEnrichment = await this.enrichContextForTool(
        toolCall,
        sessionId,
      );
  ...
  generateSearchTerms(toolCall) {
    ...
    if (toolCall.parameters) {
      ...
      if (params.command) {
        const commandParts = params.command
          .split(/\s+/)
          .filter((p) => p.length > 2);
    ...
    switch (category) {
      case "file-read":
        hints.push(
          ...
          "May reference existing code or data",
        );
        break;
      case "file-write":
        hints.push(
