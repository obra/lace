...
export class SnapshotCLI {
  ...
  async listSnapshots(options = {}) {
    try {
      ...
      const tableData = snapshots.map((snapshot) => ({
        ...
        size: snapshot.performance
          ? this.formatFileSize(snapshot.performance.snapshotSizeBytes)
          : "N/A",
      }));
  ...
  async showSystemStats() {
    try {
      ...
      this.output.log(
        `Total snapshots: ${this.colorize(stats.totalSnapshots, "number")}`,
      );
      this.output.log(
        `Total size: ${this.colorize(this.formatFileSize(stats.totalSize), "number")}`,
      );
      this.output.log(
        `Average snapshot size: ${this.colorize(this.formatFileSize(stats.averageSnapshotSize), "number")}`,
      );

      if (stats.oldestSnapshot) {
        this.output.log(
          `Oldest snapshot: ${this.colorize(this.formatTimestamp(stats.oldestSnapshot), "date")}`,
        );
      }
      if (stats.newestSnapshot) {
        this.output.log(
          `Newest snapshot: ${this.colorize(this.formatTimestamp(stats.newestSnapshot), "date")}`,
        );
      }
    } catch (error) {
  ...
  async inspectSnapshot(snapshotId, options = {}) {
    try {
      const snapshot =
        await this.snapshotManager.loadSnapshotMetadata(snapshotId);

      this.output.log(
        this.colorize(`\n🔍 Snapshot Details: ${snapshotId}`, "header"),
      );
      this.output.log(
        `Type: ${this.colorize(snapshot.type.toUpperCase(), "type")}`,
      );
      this.output.log(
        `Timestamp: ${this.colorize(this.formatTimestamp(snapshot.timestamp), "date")}`,
      );
      this.output.log(
        `Git Commit: ${this.colorize(snapshot.gitCommitSha, "commit")}`,
      );
      ...
      if (snapshot.toolCall) {
        ...
        this.output.log(
          `Tool: ${this.colorize(snapshot.toolCall.toolName, "tool")}`,
        );
        this.output.log(
          `Operation: ${this.colorize(snapshot.toolCall.operation, "operation")}`,
        );

        if (snapshot.toolCall.parameters) {
          this.output.log("Parameters:");
          Object.entries(snapshot.toolCall.parameters).forEach(
            ([key, value]) => {
      ...
      if (snapshot.executionResult) {
        ...
        this.output.log(
          `Success: ${this.colorize(snapshot.executionResult.success, snapshot.executionResult.success ? "success" : "error")}`,
        );
        this.output.log(`Duration: ${snapshot.executionResult.duration}ms`);

        if (snapshot.executionResult.error) {
          this.output.log(
            `Error: ${this.colorize(snapshot.executionResult.error, "error")}`,
          );
        }
      }
      ...
      if (snapshot.performance) {
        ...
        this.output.log(
          `Snapshot size: ${this.formatFileSize(snapshot.performance.snapshotSizeBytes)}`,
        );

        if (snapshot.performance.processingTimeMs) {
          this.output.log(
            `Processing time: ${snapshot.performance.processingTimeMs}ms`,
          );
        }
      }
      ...
      if (options.showRelated) {
        ...
        if (related.length > 0) {
          ...
          related.forEach((rel) => {
            this.output.log(
              `- ${rel.snapshotId} (${rel.type}) - ${this.formatTimestamp(rel.timestamp)}`,
            );
          });
        }
      }
  ...
  async previewRestore(snapshotId, options = {}) {
    try {
      ...
      this.output.log(
        `Snapshot: ${this.colorize(preview.snapshotInfo.description || preview.snapshotInfo.type, "info")}`,
      );
      this.output.log(
        `Target commit: ${this.colorize(preview.targetCommit, "commit")}`,
      );
      this.output.log(
        `Current commit: ${this.colorize(preview.currentCommit, "commit")}`,
      );

      // Show changes summary
      this.output.log(this.colorize("\n📝 Changes Summary", "section"));
      this.output.log(
        `Total changes: ${this.colorize(preview.summary.totalChanges, "number")}`,
      );
      this.output.log(
        `Files modified: ${this.colorize(preview.summary.filesModified, "number")}`,
      );
      this.output.log(
        `Files added: ${this.colorize(preview.summary.filesAdded, "success")}`,
      );
      this.output.log(
        `Files deleted: ${this.colorize(preview.summary.filesDeleted, "error")}`,
      );
  ...
  async previewFileRestore(snapshotId, filePaths) {
    try {
      const preview = await this.restoreOps.previewFileRestore(
        snapshotId,
        filePaths,
      );
      ...
      this.output.log(
        `Snapshot: ${this.colorize(preview.snapshotInfo.type, "info")} from ${this.formatTimestamp(preview.snapshotInfo.timestamp)}`,
      );
  ...
  async restoreFromSnapshot(snapshotId, options = {}) {
    try {
      ...
      if (this.interactive && !options.confirm) {
        if (!this.prompt) {
          this.output.warn(
            "Interactive mode requires prompt capability. Use { confirm: true } option to bypass.",
          );
          return;
        }

        const confirmed = await this.prompt.confirm(
          `Are you sure you want to restore from snapshot ${snapshotId}?`,
        );
        if (!confirmed) {
      ...
      const result = await this.restoreOps.restoreFromSnapshot(
        snapshotId,
        options,
      );
      ...
      if (result.success) {
        this.output.info(
          this.colorize("✅ Snapshot successfully restored!", "success"),
        );
        this.output.info(
          `Restored to commit: ${this.colorize(result.restoredCommit, "commit")}`,
        );
        this.output.info(`Duration: ${result.duration}ms`);

        if (result.backupBranch) {
          this.output.info(
            `Backup branch created: ${this.colorize(result.backupBranch, "branch")}`,
          );
        }
      }
    } catch (error) {
  ...
  async restoreFiles(snapshotId, filePaths, options = {}) {
    try {
      ...
      if (this.interactive && !options.confirm) {
        ...
        const confirmed = await this.prompt.confirm(
          `Restore ${filePaths.length} file(s) from snapshot ${snapshotId}?`,
        );
        if (!confirmed) {
      ...
      this.output.log(
        this.colorize(
          `\n📁 Restoring files from snapshot: ${snapshotId}`,
          "header",
        ),
      );
      ...
      if (result.success) {
        this.output.info(
          this.colorize("✅ Files restored successfully!", "success"),
        );
  ...
  async showRecommendations() {
    try {
      const recommendations =
        await this.restoreOps.getRestorationRecommendations();
  ...
  async showHelp() {
    ...
    this.output.log(this.colorize("\nCommands:", "section"));
    this.output.log(
      "  list [--type=TYPE]           List all snapshots, optionally filtered by type",
    );
    this.output.log(
      "  stats                        Show snapshot system statistics",
    );
    this.output.log(
      "  inspect <snapshotId>         Show detailed snapshot information",
    );
    this.output.log(
      "  preview <snapshotId>         Preview restoration changes",
    );
    this.output.log(
      "  restore <snapshotId>         Restore project from snapshot",
    );
    this.output.log(
      "  restore-files <snapshotId>   Restore specific files from snapshot",
    );
    this.output.log(
      "  recommendations              Show restoration recommendations",
    );
    this.output.log(
    ...
    this.output.log(
      "  --backup                     Create backup branch before restoration",
    );
    this.output.log(
      "  --related                    Show related snapshots in inspect",
    );
    this.output.log(
    ...
    this.output.log(
      "  checkpoint                   Manual snapshots created by user",
    );
    this.output.log(
      "  pre-tool                     Automatic snapshots before tool execution",
    );
    this.output.log(
      "  post-tool                    Automatic snapshots after tool execution",
    );
  }
  ...
  async showExamples() {
    ...
    this.output.log(
      "  lace snapshot list                    # List all snapshots",
    );
    this.output.log(
      "  lace snapshot list --type=checkpoint  # List only checkpoints",
    );
    this.output.log(
      "  lace snapshot stats                   # Show system statistics",
    );
    ...
    this.output.log(
      "  lace snapshot recommendations         # Get restoration suggestions",
    );
    this.output.log(
      "  lace snapshot help                    # Show help information",
    );
  }
