...
export const queueCommand: Command = {
  ...
  async execute(args: string, ui: UserInterface): Promise<void> {
    ...
    if (trimmedArgs === 'clear') {
      ...
      ui.displayMessage(`📬 Cleared ${clearedCount} user message${clearedCount === 1 ? '' : 's'} from queue`);
      return;
    }
    ...
    if (trimmedArgs === '') {
      ...
      const messages = [
        ...
        stats.oldestMessageAge ? `   Oldest: ${Math.round(stats.oldestMessageAge / 1000)}s ago` : null,
        '',
