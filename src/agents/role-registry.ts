...
export function getRole(name: string): Role {
  if (!isValidRoleName(name)) {
    ...
    throw new Error(`INVALID AGENT ROLE: '${name}'. Valid roles are: ${validRoles}`);
  }
 ...
 */
export function getRoleMetadata(): Array<{ name: string; capabilities: string[]; defaultModel: string }> {
