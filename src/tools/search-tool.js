      ...
      methods: {
        grep: {
          ...
          parameters: {
            ...
            path: { type: 'string', required: false, description: 'Path to search in (default: current directory)' },
            recursive: { type: 'boolean', required: false, description: 'Search recursively (default: true)' },
            ignoreCase: { type: 'boolean', required: false, description: 'Case insensitive search (default: true)' }
          }
        },
        find: {
          description: 'Find files and directories by name',
          parameters: {
            name: { type: 'string', required: true, description: 'Name pattern to search for' },
            path: { type: 'string', required: false, description: 'Path to search in (default: current directory)' },
            type: { type: 'string', required: false, description: 'Type to search for: file, directory, or both (default: both)' }
          }
        }
      }
    };
