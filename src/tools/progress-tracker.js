...
export class ProgressTracker {
  ...
  async updateProgress(agentId, progressUpdate) {
    ...
    const updatedProgress = {
      ...
      progressPercent:
        progressUpdate.progressPercent ?? existingProgress.progressPercent,
  ...
  getActiveAgents() {
    return Array.from(this.progressData.values()).filter(
      (progress) => !["completed", "failed"].includes(progress.status),
    );
  }
   ...
   */
  cleanupOldest() {
    const entries = Array.from(this.progressData.entries()).sort(
