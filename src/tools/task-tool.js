...
export class TaskTool {
  ...
  async delegateTask(params) {
    ...
    try {
      ...
      const taskPromise = this.agent.delegateTask(sessionId, description, {
        role,
        assignedModel: model,
  ...
  async reportProgress(params) {
    const { status, progressPercent = null, details = "" } = params;
  ...
  async requestHelp(params) {
    const { errorDescription, attemptedSolutions = [], helpNeeded } = params;
      ...
      if (this.progressTracker) {
        await this.progressTracker.updateProgress(this.agent.generation, {
          ...
          helpRequest: {
            ...
            attemptedSolutions: attemptedSolutions.map((s) =>
              s.substring(0, 100),
            ),
  ...
  async sendMessage(params) {
    const { recipientId, messageType, content, priority = "medium" } = params;
    ...
    try {
      ...
      const originalLength = messageContent.length;
      ...
      return {
        ...
        timestamp,
        contentTruncated,
        originalLength: contentTruncated
          ? originalLength
  ...
  async receiveMessages(params = {}) {
    const { messageType = null, limit = 50, markAsRead = false } = params;
    ...
    try {
      ...
      const unreadCount = markAsRead
        ? limitedMessages.filter((msg) => !msg.read).length
  ...
  cleanupMessageQueue(recipientId) {
    ...
    const filteredMessages = queue.filter(
      (msg) => now - msg.timestamp < this.messageCleanupInterval,
    );
  ...
  getSchema() {
    return {
      name: "task",
      description:
        "Agent orchestration tool for delegating tasks, spawning sub-agents, and coordinating complex workflows",
      methods: {
        delegateTask: {
          description:
            "Delegate a task to a specialized sub-agent with optional role and model specification",
          parameters: {
            description: {
              type: "string",
              required: true,
            ...
            role: {
              ...
              required: false,
              description:
                'Role for the sub-agent (e.g., "specialist", "researcher", "coder")',
            },
            model: {
              type: "string",
              required: false,
              description:
                'Model to use for the sub-agent (e.g., "claude-3-5-sonnet-20241022")',
            },
            provider: {
              type: "string",
              required: false,
        ...
        reportProgress: {
          description:
            "Send lightweight progress updates that are stored in memory (not conversation history)",
          parameters: {
            status: {
              type: "string",
              required: true,
              description:
                'Current status (e.g., "in_progress", "completed", "failed", "waiting")',
            },
            progressPercent: {
              type: "number",
              required: false,
            ...
            details: {
              ...
              required: false,
              description:
                "Brief details about current progress (keep under 50 tokens)",
            },
          },
        },
        requestHelp: {
          description:
            "Signal that the agent needs assistance with an error or problem",
          parameters: {
            errorDescription: {
              type: "string",
              required: true,
        ...
        sendMessage: {
          description:
            "Send a message to another agent for coordination without going through coordinator",
          parameters: {
            recipientId: {
              type: "string",
              required: true,
            ...
            messageType: {
              ...
              required: true,
              description:
                "Type of message: status_update, request_help, share_result, coordination",
            },
            content: {
              type: "string",
              required: false,
