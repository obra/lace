...
export class SynthesisEngine {
  ...
  analyzeSynthesisNeeds(toolResults, toolCalls) {
    ...
    for (let i = 0; i < toolResults.length; i++) {
      ...
      const threshold = this.config.toolThresholds[toolName] || this.config.defaultThreshold;
      ...
      if (estimatedTokens > threshold) {
        ...
        noSynthesis.push({ result: toolResult, index: i, reason: 'under_threshold' });
      }
    }
  ...
  async processSynthesis(toolResults, toolCalls, synthesisFunction) {
    ...
    if (analysis.shouldUseBatch) {
      await this.processBatchSynthesis(analysis.needsSynthesis, synthesizedResults, synthesisFunction);
    } else {
      await this.processIndividualSynthesis(analysis.needsSynthesis, synthesizedResults, synthesisFunction);
    }
   ...
   */
  async processBatchSynthesis(needsSynthesis, synthesizedResults, synthesisFunction) {
    ...
    for (let i = 0; i < needsSynthesis.length; i += batchSize) {
      ...
      if (batch.length === 1) {
        ...
        const item = batch[0];
        const synthesized = await synthesisFunction.individual(item.result, item.call);
   ...
   * @param {Function} synthesisFunction - Synthesis function
   */
  async processIndividualSynthesis(needsSynthesis, synthesizedResults, synthesisFunction) {
...
Please provide a synthesized summary for each tool, maintaining their individual identity while noting relationships. Return as a JSON array with ${toolBatch.length} summaries in the same order.`;
  ...
  fallbackParseBatchSynthesis(content, expectedCount) {
    ...
    for (let i = 0; i < expectedCount; i++) {
      ...
      const end = i === expectedCount - 1 ? content.length : (i + 1) * avgLength;
      summaries.push(content.slice(start, end).trim() || `Tool ${i + 1} synthesis unavailable`);
    }
  ...
  getSynthesisStats(toolResults, synthesizedResults) {
    ...
    return {
      ...
      tokenReduction: this.calculateTokenReduction(toolResults, synthesizedResults),
  ...
  calculateTokenReduction(original, synthesized) {
    ...
    const synthesizedTexts = synthesized.map(r => 
      r.synthesized ? (r.summary || this.resultExtractor.extract(r)) : this.resultExtractor.extract(r)
    );

    const originalTokens = this.tokenEstimator.estimateTotal(originalTexts);
    const synthesizedTokens = this.tokenEstimator.estimateTotal(synthesizedTexts);

    return {
      originalTokens,
      synthesizedTokens,
      reduction: originalTokens - synthesizedTokens,
      reductionPercent: originalTokens > 0 ? ((originalTokens - synthesizedTokens) / originalTokens) * 100 : 0
    };
  }
}