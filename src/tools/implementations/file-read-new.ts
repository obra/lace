...
export class FileReadTool extends Tool {
  ...
  protected async executeValidated(
    ...
    try {
      ...
      if (!args.startLine && !args.endLine) {
        await this.validateFileSizeForWholeRead(args.path);
      }
  ...
  private async validateFileSizeForWholeRead(filePath: string): Promise<void> {
    try {
      const fileStats = await stat(filePath);
      if (fileStats.size > MAX_FILE_SIZE) {
        const fileSizeFormatted = this.formatFileSize(fileStats.size);
        throw new Error(
          `File is too large (${fileSizeFormatted}) for whole-file read. Use startLine and endLine parameters for ranged reads (e.g., startLine: 1, endLine: 100). File size limit is ${this.formatFileSize(
            MAX_FILE_SIZE
