...
describe('Multi-Agent Task Manager Integration', () => {
  ...
  beforeEach(() => {
    tempDbPath = path.join(os.tmpdir(), `lace-tasks-integration-${Date.now()}.db`);
  ...
  describe('Multi-agent task workflow', () => {
    it('should support full lifecycle of multi-agent task collaboration', async () => {
      ...
      const listTool = new TaskListTool(persistence);
      const statusTool = new TaskUpdateStatusTool(persistence);
      const reassignTool = new TaskReassignTool(persistence);
      ...
      const assignResult = await reassignTool.execute({
        taskId,
  ...
  describe('New agent assignment workflow', () => {
    it('should handle task assignment to new agent specification', async () => {
      ...
      const actualAgentResult = await reassignTool.execute({
        taskId,
