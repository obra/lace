  ...
  describe('AnthropicProvider', () => {
    ...
    describe('Session Tracking', () => {
      ...
      test('should track multiple conversations in same provider', () => {
        ...
        const session1 = provider.getOrCreateSessionId(null, [{ role: 'user', content: 'first' }]);
        const session2 = provider.getOrCreateSessionId(null, [{ role: 'user', content: 'second' }]);
        const session3 = provider.getOrCreateSessionId(null, [{ role: 'user', content: 'first' }]); // Same as first
    ...
    describe('Chat Method Integration', () => {
      test('should generate session ID in chat method', async () => {
        ...
        try {
          ...
          assert.strictEqual(error.message, 'OpenAI provider not yet implemented');
        }
    ...
    describe('Chat Method Integration', () => {
      test('should generate session ID in chat method', async () => {
        ...
        try {
          ...
          assert.strictEqual(error.message, 'Local provider not yet implemented');
        }
