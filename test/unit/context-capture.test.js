...
describe('ContextCapture', () => {
  ...
  describe('constructor', () => {
    test('should create ContextCapture with database dependencies', async () => {
      ...
      const capture = new ContextCapture(mockConversationDB, mockActivityLogger);
    ...
    test('should accept optional configuration', async () => {
      ...
      const capture = new ContextCapture(mockConversationDB, mockActivityLogger, config);
  ...
  describe('conversation context capture', () => {
    test('should capture recent conversation history', async () => {
      ...
      const capture = new ContextCapture(mockConversationDB, mockActivityLogger);
      const context = await capture.captureConversationContext('session-123', 1);

      assert.ok(context.recentHistory, 'Should have recent history');
      assert.ok(Array.isArray(context.recentHistory), 'Recent history should be array');
      assert.strictEqual(context.recentHistory.length, 2, 'Should have 2 history entries');
    ...
    test('should limit conversation history to configured amount', async () => {
      ...
      assert.strictEqual(context.recentHistory.length, 1, 'Should limit to 1 entry');
    });
    ...
    test('should handle missing conversation data gracefully', async () => {
      ...
      const capture = new ContextCapture(emptyConversationDB, mockActivityLogger);
      const context = await capture.captureConversationContext('nonexistent-session', 1);

      assert.ok(Array.isArray(context.recentHistory), 'Should return empty array');
      assert.strictEqual(context.recentHistory.length, 0, 'Should have no history entries');
    });
  });
  ...
  describe('activity context capture', () => {
    test('should capture recent tool usage', async () => {
      ...
      assert.ok(Array.isArray(context.recentToolUses), 'Tool uses should be array');
      assert.strictEqual(context.recentToolUses.length, 2, 'Should have 2 activity entries');
    ...
    test('should filter tool events by session', async () => {
      ...
      const capture = new ContextCapture(mockConversationDB, mixedActivityLogger);
      const context = await capture.captureActivityContext('session-123');

      assert.strictEqual(context.recentToolUses.length, 1, 'Should filter by session');
      assert.strictEqual(context.recentToolUses[0].data.toolName, 'correct-session');
    });
    ...
    test('should limit tool usage to configured amount', async () => {
      ...
      assert.strictEqual(context.recentToolUses.length, 1, 'Should limit to 1 entry');
    });
  });
  ...
  describe('real repository integration', () => {
    test('should capture real repository SHA when available', async () => {
      ...
      assert.notStrictEqual(repoSha, 'unknown', 'Should not be unknown in git repo');
    });
    ...
    test('should handle non-git directories gracefully', async () => {
      ...
      assert.strictEqual(repoSha, 'unknown', 'Should return unknown for non-git directory');
    });
  });
  ...
  describe('comprehensive context capture', () => {
    test('should capture complete context for tool execution', async () => {
      ...
      const context = await capture.captureFullContext('session-123', 1, toolCall, testDir);
      ...
      // Verify tool context
      assert.deepStrictEqual(context.currentTool, toolCall, 'Should have current tool info');
  ...
  describe('context enrichment', () => {
    test('should add semantic context based on tool type', async () => {
      ...
      const context = await capture.enrichContextForTool(fileToolCall, 'session-123');
    ...
    test('should suggest relevant search terms', async () => {
      ...
      assert.ok(Array.isArray(searchTerms), 'Should return array of search terms');
  ...
  describe('performance and caching', () => {
    test('should cache repeated context captures', async () => {
      ...
      const capture = new ContextCapture(countingConversationDB, mockActivityLogger);
      ...
      assert.ok(secondCallCount >= firstCallCount, 'Should not make fewer calls');
    });
    ...
    test('should handle database errors gracefully', async () => {
      ...
      const failingConversationDB = {
        getConversationHistory: async () => { throw new Error('Database connection failed'); },
        searchConversations: async () => { throw new Error('Database connection failed'); }
      };

      const capture = new ContextCapture(failingConversationDB, mockActivityLogger);
      
      // Should not throw, but return degraded context
      const context = await capture.captureConversationContext('session-123', 1);
      
      assert.ok(context, 'Should return context object even on DB failure');
      assert.ok(Array.isArray(context.recentHistory), 'Should return empty history on failure');
    });
  });
});