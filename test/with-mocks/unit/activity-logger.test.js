...
describe("ActivityLogger", () => {
  ...
  describe("Event Logging", () => {
    ...
    test("should handle null model session id", async () => {
      await logger.logEvent("test_event", "session-123", null, {
        data: "test",
      });
    ...
    test("should not throw on logging errors", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "ActivityLogger: Database not initialized",
      );
  ...
  describe("Event Querying", () => {
    beforeEach(async () => {
      ...
      await logger.logEvent("user_input", "session-1", "model-1", {
        content: "Hello",
      });
      await logger.logEvent("agent_response", "session-1", "model-1", {
        content: "Hi there",
      });
      await logger.logEvent("user_input", "session-2", "model-2", {
        content: "Goodbye",
      });

      // Wait a moment to ensure different timestamps
      await new Promise((resolve) => setTimeout(resolve, 10));
      await logger.logEvent("tool_execution", "session-1", "model-1", {
        tool: "file-tool",
      });
    });
