...
describe("SnapshotCLI", () => {
  ...
  beforeEach(async () => {
    ...
    mockRestoreOperations = {
      listAvailableSnapshots: async (filters) =>
        mockSnapshotManager.listSnapshots(filters),
      getSnapshotDetails: async (snapshotId) =>
        mockSnapshotManager.loadSnapshotMetadata(snapshotId),
  ...
  describe("constructor", () => {
    ...
    test("should validate required dependencies", async () => {
      ...
      try {
        ...
        assert.ok(
          error.message.includes("RestoreOperations"),
          "Should validate restore operations",
        );
      }
    });
  });
  ...
  describe("list command", () => {
    test("should list all snapshots in table format", async () => {
      ...
      assert.strictEqual(
        tableOutput.data.length,
        3,
        "Should show all snapshots",
      );
    ...
    test("should filter snapshots by type", async () => {
      ...
      assert.strictEqual(
        tableOutput.data.length,
        1,
        "Should filter to checkpoints only",
      );
    ...
    test("should show system statistics", async () => {
      ...
      assert.ok(
        statsText.includes("Total snapshots"),
  ...
  describe("inspect command", () => {
    test("should show detailed snapshot information", async () => {
      ...
      assert.ok(
        allText.includes("2025-06-05T15-30-00-checkpoint"),
        "Should show snapshot ID",
      );
      assert.ok(allText.includes("checkpoint"), "Should show snapshot type");
      assert.ok(
        allText.includes("Before major refactoring"),
    ...
    test("should show related snapshots", async () => {
      ...
      assert.ok(
        allText.includes("Related"),
        "Should show related snapshots section",
      );
      assert.ok(
        allText.includes("post-tool"),
        "Should show related post-tool snapshot",
      );
    });
    ...
    test("should handle missing snapshot gracefully", async () => {
      ...
      assert.ok(
        errorOutputs[0].message.includes("not found"),
  ...
  describe("restore preview command", () => {
    test("should preview full restoration changes", async () => {
      ...
      assert.ok(
        allText.includes("4") && allText.includes("changes"),
        "Should show total changes",
      );
    ...
    test("should preview file restoration", async () => {
      ...
      await cli.previewFileRestore("2025-06-05T15-30-00-checkpoint", [
        "src/main.js",
        "package.json",
      ]);
      ...
      assert.ok(
        allText.includes("File Restore Preview"),
        "Should show file restore header",
      );
      assert.ok(
        allText.includes("src/main.js"),
        "Should list files to restore",
      );
      assert.ok(
        allText.includes("package.json"),
        "Should list all requested files",
      );
    });
    ...
    test("should show safety warnings", async () => {
      ...
      const cli = new SnapshotCLI(mockSnapshotManager, unsafeRestoreOps, {
        output: mockOutput,
      });
      ...
      assert.ok(
        warningText.includes("working tree"),
        "Should warn about working tree changes",
      );
    });
  });
  ...
  describe("restore command", () => {
    test("should restore from snapshot with confirmation", async () => {
      ...
      assert.ok(
        infoText.includes("successfully restored"),
        "Should confirm restoration success",
      );
    ...
    test("should restore specific files", async () => {
      ...
      assert.ok(
        infoText.includes("Files restored"),
        "Should confirm file restoration",
      );
    ...
    test("should handle restoration errors gracefully", async () => {
      ...
      assert.ok(errorOutputs.length > 0, "Should output error message");
      assert.ok(
        errorOutputs[0].message.includes("Restoration failed"),
  ...
  describe("recommendations command", () => {
    test("should show restoration recommendations", async () => {
      ...
      assert.ok(
        allText.includes("Recommendations"),
      ...
      if (tableOutput) {
        ...
        assert.ok(
          tableText.includes("manual checkpoint"),
          "Should show recommendation reasons in table",
        );
        assert.ok(
          tableText.includes("HIGH"),
          "Should show priority levels in table",
        );
      } else {
        assert.ok(
          allText.includes("manual checkpoint"),
    ...
    test("should format recommendations with priorities", async () => {
      ...
      assert.ok(
        tableOutput.data.length >= 2,
        "Should show multiple recommendations",
      );

      const highPriorityRec = tableOutput.data.find(
        (r) => r.priority === "HIGH",
      );
  ...
  describe("help and usage", () => {
    test("should show help information", async () => {
      ...
      assert.ok(
        helpText.includes("inspect"),
        "Should document inspect command",
      );
      assert.ok(
        helpText.includes("restore"),
        "Should document restore command",
      );
    });
    ...
    test("should show command examples", async () => {
      ...
      assert.ok(
        exampleText.includes("Examples"),
        "Should show examples header",
      );
      assert.ok(
        exampleText.includes("lace snapshot"),
  ...
  describe("output formatting", () => {
    ...
    test("should format file sizes in human-readable format", async () => {
      ...
      assert.ok(
        largeFormatted.includes("MB"),
        "Should use MB for larger files",
      );
    });
    ...
    test("should colorize output based on type and priority", async () => {
      ...
      assert.ok(
        ...
        "Should return colored error text",
      );
      assert.ok(
        typeof successText === "string",
        "Should return colored success text",
      );
      assert.ok(
        typeof warningText === "string",
        "Should return colored warning text",
      );
    });
  });
});
