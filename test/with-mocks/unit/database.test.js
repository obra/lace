...
import { test, describe, beforeEach, afterEach, TestHarness, assert } from '../../test-harness.js'
...
describe('Conversation Database', () => {
  ...
  describe('Message Storage', () => {
    test('should save and retrieve messages', async () => {
      ...
      assert.strictEqual(history[0].content, 'Hi there!', 'Should preserve content')
    ...
    test('should save tool calls with messages', async () => {
      ...
      await db.saveMessage('test-session', 0, 'assistant', 'Reading file...', toolCalls, 1500)

      const history = await db.getConversationHistory('test-session')
      const message = history[0]

      assert.strictEqual(message.context_size, 1500, 'Should save context size')

      const savedToolCalls = JSON.parse(message.tool_calls)
      assert.ok(Array.isArray(savedToolCalls), 'Should parse tool calls as array')
      assert.strictEqual(savedToolCalls[0].name, 'file_read', 'Should preserve tool call data')
  ...
  describe('Session Management', () => {
    test('should isolate conversations by session', async () => {
      ...
      assert.strictEqual(session1History[0].content, 'Message in session 1', 'Should have correct content')
      assert.strictEqual(session2History[0].content, 'Message in session 2', 'Should have correct content')
    ...
    test('should limit conversation history retrieval', async () => {
      ...
      assert.strictEqual(limitedHistory.length, 5, 'Should limit to requested number')
      assert.strictEqual(fullHistory.length, 10, 'Should return all messages when limit is high')
  ...
  describe('Search Functionality', () => {
    test('should search conversation content', async () => {
      ...
      await db.saveMessage(sessionId, 0, 'user', 'How do I calculate fibonacci?')
      await db.saveMessage(sessionId, 0, 'assistant', 'You can use recursion or iteration')
      ...
      assert.ok(fibResults[0].content.includes('fibonacci'), 'Should match search term')

      assert.strictEqual(primeResults.length, 1, 'Should find prime message')
      assert.ok(primeResults[0].content.includes('prime'), 'Should match search term')
  ...
  describe('Generation Tracking', () => {
    test('should track agent generations', async () => {
      ...
      assert.strictEqual(gen0History.length, 1, 'Should have generation 0 message')
      assert.strictEqual(gen1History.length, 1, 'Should have generation 1 message')
      assert.strictEqual(gen0History[0].generation, 0, 'Should have correct generation')
      assert.strictEqual(gen1History[0].generation, 1, 'Should have correct generation')
