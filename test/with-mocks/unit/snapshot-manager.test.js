...
describe("SnapshotManager", () => {
  ...
  describe("constructor", () => {
    test("should create SnapshotManager with valid project path", async () => {
      ...
      assert.strictEqual(
        manager.snapshotsDir,
        join(testDir, ".lace", "snapshots"),
      );
      assert.strictEqual(
        manager.metadataDir,
        join(testDir, ".lace", "snapshots", "metadata"),
      );
    });
  ...
  describe("initialization", () => {
    test("should initialize directory structure and git operations", async () => {
      ...
      assert.ok(
        await utils.fileExists(join(testDir, ".lace", "snapshots", "metadata")),
      );
    ...
    test("should load existing configuration", async () => {
      ...
      await utils.writeFile(
        configPath,
        JSON.stringify({ enabled: false, custom: "value" }),
      );
  ...
  describe("snapshot creation", () => {
    test("should create pre-tool snapshot with metadata", async () => {
      ...
      assert.ok(
        snapshot.snapshotId.includes("pre-tool"),
        "Should be pre-tool snapshot",
      );
    ...
    test("should create post-tool snapshot with execution results", async () => {
      ...
      assert.ok(
        snapshot.snapshotId.includes("post-tool"),
        "Should be post-tool snapshot",
      );
      assert.strictEqual(snapshot.type, "post-tool");
      assert.deepStrictEqual(snapshot.executionResult, executionResult);
      assert.ok(
        snapshot.performance.processingTimeMs >= 0,
    ...
    test("should create manual checkpoint snapshot", async () => {
      ...
      assert.ok(
        snapshot.snapshotId.includes("checkpoint"),
        "Should be checkpoint snapshot",
      );
      ...
      try {
        ...
        assert.ok(
          error.message.includes("Git operation failed") ||
            error.message.includes("Failed to create checkpoint"),
  ...
  describe("metadata management", () => {
    test("should save and load snapshot metadata", async () => {
      ...
      const loadedMetadata = await manager.loadSnapshotMetadata(
        snapshot.snapshotId,
      );
  ...
  describe("configuration management", () => {
    test("should apply retention policies", async () => {
      ...
      assert.ok(
        ...
        "Should respect maxSnapshots limit",
      );
    });
