...
describe('Dual Logging System Integration', () => {
  ...
  async function setup() {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'lace-dual-logging-test-'));
  ...
  describe('Independent Logging Systems', () => {
    test('should log to both systems without interference', async () => {
      ...
      try {
        ...
        const modelRequest = activityEvents.find(e => e.event_type === 'model_request');
        const modelResponse = activityEvents.find(e => e.event_type === 'model_response');
        ...
        const debugLogExists = await fs.access(debugLogFile).then(() => true).catch(() => false);
    ...
    test('should handle activity logging failure gracefully without affecting debug logging', async () => {
      ...
      try {
        ...
        const debugLogExists = await fs.access(debugLogFile).then(() => true).catch(() => false);
        assert.ok(debugLogExists, 'Debug log file should exist even when activity logging fails');
        
        const debugLogContent = await fs.readFile(debugLogFile, 'utf8');
        assert.ok(debugLogContent.length > 0, 'Debug log should have content even when activity logging fails');
    ...
    test('should handle debug logging failure without affecting activity logging', async () => {
      ...
      try {
        ...
        const activityEvents = await activityLogger.getEvents({ sessionId });
        assert.ok(activityEvents.length > 0, 'Activity logging should work even when debug logging fails');
        
        const modelRequest = activityEvents.find(e => e.event_type === 'model_request');
        assert.ok(modelRequest, 'Should have model_request event even when debug logging fails');
  ...
  describe('Subagent Logger Inheritance', () => {
    test('should pass both loggers to subagents', async () => {
      ...
      try {
        ...
        assert.ok(subagent.activityLogger, 'Subagent should have activity logger');
  ...
  describe('Performance and Independence', () => {
    test('should not significantly impact performance when both systems enabled', async () => {
      ...
      try {
        ...
        assert.ok(overhead < 200, `Logging overhead (${overhead}ms) should be minimal`);
