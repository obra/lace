...
describe("LaceUI Activity Logging Methods", () => {
  ...
  describe("Streaming Token Logging", () => {
    ...
    test("should handle streaming without breaking on errors", async () => {
      ...
      await expect(
        activityLogger.logEvent("streaming_token", sessionId, null, {
          token: "test",
          timestamp: new Date().toISOString(),
        }),
  ...
  describe("Activity Retrieval Methods", () => {
    ...
    test("getSessionActivity should filter by session ID", async () => {
      ...
      await activityLogger.logEvent("user_input", "session-456", null, {
      ...
      const otherSessionEvents = events.filter(
        (e) => JSON.parse(e.data).content === "other session",
      );
      expect(otherSessionEvents).toHaveLength(0);
    });

    test("getActivityByType should filter by event type", async () => {
      const userInputEvents = await activityLogger.getEvents({
        eventType: "user_input",
      });
      ...
      const toolEvents = await activityLogger.getEvents({
        eventType: "tool_execution",
      });
  ...
  describe("Activity Logger Cleanup", () => {
    ...
    test("should handle reinitialization", async () => {
      ...
      await activityLogger.logEvent("test", "session-123", null, {
        test: "data",
      });
  ...
  describe("Event Data Validation", () => {
    test("should handle various data types", async () => {
      const testCases = [
        ...
        {
          type: "object_data",
          data: { key: "value", nested: { deep: "object" } },
        },
      ...
      expect(JSON.parse(objectEvent!.data)).toEqual({
        key: "value",
        nested: { deep: "object" },
      });
    });
    ...
    test("should handle empty and null data", async () => {
      ...
      const emptyStringEvent = events.find(
        (e) => e.event_type === "empty_string",
      );
