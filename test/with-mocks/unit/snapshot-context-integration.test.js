...
describe("SnapshotManager + ContextCapture Integration", () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      const contextModule = await import(
        "../../../src/snapshot/context-capture.js"
      );
  ...
  describe("Integrated Context Capture", () => {
    test("should enrich snapshot metadata with conversation context", async () => {
      ...
      assert.ok(
        snapshot.context.recentHistory,
        "Should have conversation history",
      );
      assert.ok(
        snapshot.context.recentToolUses,
        "Should have tool usage history",
      );
      ...
      // Verify enrichment
      assert.ok(
        snapshot.context.toolCategory,
        "Should have tool categorization",
      );
      assert.ok(snapshot.context.semanticHints, "Should have semantic hints");
      assert.ok(
        snapshot.context.contextKeywords,
    ...
    test("should fall back gracefully when context capture fails", async () => {
      ...
      assert.strictEqual(
        ...
        "session-789",
        "Should have session ID from degraded context",
      );
    });
