...
describe('RestoreOperations', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      const module = await import('../../../src/snapshot/restore-operations.js')
  ...
  describe('constructor', () => {
    test('should create RestoreOperations with snapshot manager and git operations', async () => {
      ...
      const restore = new RestoreOperations(mockSnapshotManager, mockGitOperations, testDir)
    ...
    test('should validate required dependencies', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('GitOperations'), 'Should validate git operations')
      }
    })
  })
  ...
  describe('snapshot browsing', () => {
    ...
    test('should filter snapshots by type', async () => {
      ...
      const checkpoints = await restore.listAvailableSnapshots({ type: 'checkpoint' })
      assert.strictEqual(checkpoints.length, 1, 'Should filter by checkpoint type')
      assert.strictEqual(checkpoints[0].type, 'checkpoint')

      const toolSnapshots = await restore.listAvailableSnapshots({ type: 'pre-tool' })
      assert.strictEqual(toolSnapshots.length, 1, 'Should filter by pre-tool type')
    ...
    test('should get detailed snapshot information', async () => {
      ...
      const details = await restore.getSnapshotDetails('2025-06-05T15-35-00-pre-tool-file123')
    ...
    test('should handle missing snapshot gracefully', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('not found'), 'Should indicate snapshot not found')
      }
    })
  })
  ...
  describe('restoration preview', () => {
    test('should preview changes for full restore', async () => {
      ...
      const preview = await restore.previewRestore('2025-06-05T15-30-00-checkpoint')
    ...
    test('should preview selective file restoration', async () => {
      ...
      assert.strictEqual(preview.files.length, 2, 'Should match requested files')
    ...
    test('should validate files exist in snapshot', async () => {
      ...
      try {
        ...
        assert.ok(error.message.includes('not found'), 'Should indicate file not found')
      }
    })
  })
  ...
  describe('safety checks', () => {
    test('should detect working tree changes before restore', async () => {
      ...
      assert.ok(safetyCheck.hasWorkingTreeChanges, 'Should detect working tree changes')
      assert.ok(safetyCheck.workingTreeStatus, 'Should provide working tree status')
      assert.ok(safetyCheck.recommendations, 'Should provide safety recommendations')
    })
    ...
    test('should recommend stashing or committing changes', async () => {
      ...
      assert.ok(Array.isArray(safetyCheck.recommendations), 'Should provide recommendations array')
      assert.ok(safetyCheck.recommendations.length > 0, 'Should have safety recommendations')
    })
  ...
  describe('full restoration', () => {
    test('should restore complete project state from snapshot', async () => {
      ...
      const restore = new RestoreOperations(mockSnapshotManager, cleanGitOps, testDir)

      const result = await restore.restoreFromSnapshot('2025-06-05T15-30-00-checkpoint')
    ...
    test('should create backup before restoration', async () => {
      ...
      assert.ok(result.backupBranch.startsWith('backup-'), 'Should use backup prefix')
    })
    ...
    test('should handle restoration errors gracefully', async () => {
      ...
      const restore = new RestoreOperations(mockSnapshotManager, failingGitOps, testDir)

      try {
        await restore.restoreFromSnapshot('2025-06-05T15-30-00-checkpoint', { force: true })
        assert.fail('Should handle git errors')
      } catch (error) {
        assert.ok(error.message.includes('restoration failed') || error.message.includes('Git checkout failed'), 'Should provide restoration error context')
      }
    })
  })
  ...
  describe('selective file restoration', () => {
    test('should restore specific files from snapshot', async () => {
      ...
      assert.strictEqual(result.restoredFiles.length, 2, 'Should restore requested files')
    })
    ...
    test('should handle partial file restoration failures', async () => {
      ...
      try {
        await restore.restoreFiles(
          '2025-06-05T15-35-01-post-tool-file123',
          ['src/main.js', 'nonexistent.js']
        )
        assert.fail('Should handle file restoration errors')
      } catch (error) {
        assert.ok(error.message.includes('File not found'), 'Should indicate file restoration failure')
      }
    })
  })
  ...
  describe('restoration history and rollback', () => {
    test('should track restoration operations', async () => {
      ...
      await restore.restoreFromSnapshot(
        '2025-06-05T15-30-00-checkpoint',
        { force: true }
      )
      ...
      assert.ok(Array.isArray(history), 'Should return restoration history array')
    ...
    test('should support rollback to previous state', async () => {
      ...
      assert.ok(rollbackResult.restoredCommit, 'Should restore to previous commit')
      assert.ok(rollbackResult.rollbackTimestamp, 'Should track rollback timestamp')
    })
  })
  ...
  describe('integration with snapshot system', () => {
    test('should find related snapshots by tool execution', async () => {
      ...
      assert.ok(Array.isArray(relatedSnapshots), 'Should return related snapshots array')
      // Should find the corresponding post-tool snapshot
      const postToolSnapshot = relatedSnapshots.find(s => s.type === 'post-tool')
    ...
    test('should provide restoration recommendations', async () => {
      ...
      assert.ok(recommendations.length > 0, 'Should provide restoration recommendations')
