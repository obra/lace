...
describe("GitOperations", () => {
  ...
  describe("constructor", () => {
    test("should create GitOperations with valid project path", async () => {
      ...
      assert.strictEqual(
        gitOps.gitDir,
        join(testDir, ".lace", "history-snapshot-dotgit"),
      );
  ...
  describe("initialization", () => {
    ...
    test("should create proper .gitignore in snapshot repo", async () => {
      ...
      if (await utils.fileExists(gitignorePath)) {
        ...
        assert.ok(
          gitignoreContent.includes(".git/"),
          "Should exclude main .git directory",
        );
        assert.ok(
          gitignoreContent.includes(".lace/"),
          "Should exclude .lace directory",
        );
      }
    });
  });
  ...
  describe("git commands", () => {
    ...
    test("should support atomic operations", async () => {
      ...
      const commitSha = await gitOps.addAndCommit(
  ...
  describe("repository maintenance", () => {
    test("should provide repository statistics", async () => {
      ...
      assert.ok(
        typeof stats.repositorySize === "number",
        "Should have repository size",
      );
    });
    ...
    test("should support cleanup operations", async () => {
      ...
      assert.ok(
        statsAfter.repositorySize <= statsBefore.repositorySize * 1.1,
  ...
  describe("error handling", () => {
    test("should handle corrupted repository", async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          "Should indicate repository corruption",
        );
      }
    });
    ...
    test("should validate git availability", async () => {
      ...
      assert.strictEqual(
        isAvailable,
        true,
        "Git should be available in development environment",
      );
    });
  });
  ...
  describe("file operations", () => {
    test("should track file changes correctly", async () => {
      ...
      assert.ok(
        changes.modified.includes("existing-file.txt"),
        "Should detect modified file",
      );
      assert.ok(
        changes.untracked.includes("another-new.txt"),
