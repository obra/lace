...
describe("DebugLogger", () => {
  ...
  describe("Message Formatting", () => {
    ...
    test("should pad level strings to 5 characters", () => {
      ...
      assert.ok(
        logger.formatMessage("debug", "msg", timestamp).includes("[DEBUG]"),
      );
      assert.ok(
        logger.formatMessage("info", "msg", timestamp).includes("[INFO ]"),
      );
      assert.ok(
        logger.formatMessage("warn", "msg", timestamp).includes("[WARN ]"),
      );
      assert.ok(
        logger.formatMessage("error", "msg", timestamp).includes("[ERROR]"),
      );
    });
  });
