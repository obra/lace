...
describe("Tool System", () => {
  ...
  describe("ToolRegistry", () => {
    ...
    test("should execute tool methods", async () => {
      ...
      const result = await registry.callTool("read_file", {
        path: tempFile,
      });
  ...
  describe("JavaScriptTool", () => {
    test("should evaluate simple expressions", async () => {
      ...
      const result = await tool.execute({ code: "2 + 3" });
    ...
    test("should handle errors gracefully", async () => {
      ...
      const result = await tool.execute( { code: "undefined.property" });
    ...
    test("should provide console output", async () => {
      ...
      const result = await tool.execute({ code: 'console.log("test"); 42' });
      assert.ok(result.success, "Evaluation should succeed");
  ...
  describe("File Tools", () => {
    test("should read and write files", async () => {
      ...
      // Read content
      const readResult = await readTool.execute({ path: tempFile });
