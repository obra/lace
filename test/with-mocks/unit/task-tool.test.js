...
describe("TaskTool", () => {
  ...
  describe("Initialization", () => {
    test("should create TaskTool instance", async () => {
      ...
      assert.equal(
        typeof taskTool.delegateTask,
        "function",
        "Should have delegateTask method",
      );
      assert.equal(
        typeof taskTool.spawnAgent,
        "function",
        "Should have spawnAgent method",
      );
      assert.equal(
        typeof taskTool.reportProgress,
        "function",
        "Should have reportProgress method",
      );
      assert.equal(
        typeof taskTool.requestHelp,
        "function",
        "Should have requestHelp method",
      );
    });
    ...
    test("should have proper schema", async () => {
      ...
      assert.ok(
        schema.methods.delegateTask,
        "Schema should include delegateTask",
      );
      assert.ok(schema.methods.spawnAgent, "Schema should include spawnAgent");
      assert.ok(
        schema.methods.reportProgress,
        "Schema should include reportProgress",
      );
      assert.ok(
        schema.methods.requestHelp,
        "Schema should include requestHelp",
      );
    });
    ...
    test("should set progress tracker correctly", async () => {
      ...
      assert.equal(
        taskTool.progressTracker,
        mockProgressTracker,
        "Progress tracker should be set correctly",
      );
    });
  });
  ...
  describe("delegateTask", () => {
    ...
    test("should report progress on completion", async () => {
      ...
      assert.equal(
        mockProgressTracker.lastUpdate.agentId,
        1.1,
        "Should track correct agent ID",
      );
      assert.equal(
        mockProgressTracker.lastUpdate.progressData.status,
        "completed",
        "Should report completed status",
      );
      assert.equal(
        mockProgressTracker.lastUpdate.progressData.progressPercent,
        100,
    ...
    test("should fail without agent context", async () => {
      ...
      assert.equal(result.success, false, "Should fail");
      assert.ok(
        result.error.includes("agent context"),
        "Error should mention missing agent context",
      );
    });
    ...
    test("should handle agent errors", async () => {
      ...
      assert.equal(result.success, false, "Should fail");
      assert.ok(
        result.error.includes("Agent delegation failed"),
  ...
  describe("spawnAgent", () => {
    ...
    test("should truncate long task descriptions in result", async () => {
      ...
      assert.equal(result.success, true, "Should succeed");
      assert.ok(
        result.metadata.task.length <= 103,
        "Task should be truncated in metadata",
      );
      assert.ok(
        result.metadata.task.endsWith("..."),
  ...
  describe("reportProgress", () => {
    ...
    test("should report progress successfully", async () => {
      ...
      assert.ok(
        mockProgressTracker.lastUpdate,
        "Should have updated progress tracker",
      );
      assert.equal(
        mockProgressTracker.lastUpdate.progressData.status,
        "in_progress",
    ...
    test("should work without progress tracker", async () => {
      ...
      assert.equal(
        result.success,
        true,
        "Should succeed even without progress tracker",
      );
    });
    ...
    test("should fail without status", async () => {
      ...
      assert.ok(
        ...
        "Error should mention missing status",
      );
    });
    ...
    test("should truncate long details", async () => {
      ...
      assert.ok(
        mockProgressTracker.lastUpdate.progressData.details.length <= 200,
  ...
  describe("requestHelp", () => {
    ...
    test("should request help successfully", async () => {
      ...
      assert.equal(
        ...
        "Should return error description",
      );
      ...
      assert.equal(
        mockProgressTracker.lastUpdate.progressData.status,
        "needs_help",
        "Should set needs_help status",
      );
      assert.ok(
        mockProgressTracker.lastUpdate.progressData.helpRequest,
        "Should include help request data",
      );
    });
    ...
    test("should work without attempted solutions", async () => {
      ...
      assert.equal(result.success, true, "Should succeed");
      assert.equal(
        result.attemptedSolutions.length,
        0,
        "Should have empty attempted solutions",
      );
    });
    ...
    test("should fail without required parameters", async () => {
      ...
      assert.equal(
        resultNoError.success,
        false,
        "Should fail without error description",
      );

      const paramsNoHelp = { errorDescription: "Something broke" };
      const resultNoHelp = await taskTool.requestHelp(paramsNoHelp);
      assert.equal(
        resultNoHelp.success,
        false,
        "Should fail without help needed",
      );
    });
    ...
    test("should truncate long text fields", async () => {
      ...
      const helpRequest =
        mockProgressTracker.lastUpdate.progressData.helpRequest;
      assert.ok(
        helpRequest.errorDescription.length <= 500,
        "Error description should be truncated",
      );
      assert.ok(
        helpRequest.helpNeeded.length <= 200,
        "Help needed should be truncated",
      );
      assert.ok(
        helpRequest.attemptedSolutions[0].length <= 100,
  ...
  describe("Integration with ToolRegistry", () => {
    ...
    test("should handle agent context injection via registry", async () => {
      ...
      assert.equal(
        tool.agent,
        mockAgent,
        "Agent context should be set through registry",
      );
      assert.equal(
        tool.currentSessionId,
        "test-session",
        "Session ID should be set through registry",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    test("should handle progress tracker errors gracefully", async () => {
      ...
      assert.equal(
        result.success,
        false,
        "Should fail when progress tracker throws",
      );
      assert.ok(
        result.error.includes("Progress tracker failed"),
        "Should include tracker error",
      );
    });
    ...
    test("should handle subagent spawn failures", async () => {
      ...
      assert.equal(
        result.success,
        false,
        "Should fail when subagent spawn throws",
      );
      assert.ok(
        result.error.includes("Subagent spawn failed"),
        "Should include spawn error",
      );
    });
  });
});
