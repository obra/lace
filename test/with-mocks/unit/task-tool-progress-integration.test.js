...
describe("TaskTool-ProgressTracker Integration", () => {
  ...
  describe("Progress Reporting Integration", () => {
    test("should report progress through TaskTool to ProgressTracker", async () => {
      ...
      assert.equal(
        ...
        "in_progress",
        "Should store correct status",
      );
      assert.equal(
        storedProgress.progressPercent,
        75,
        "Should store correct progress",
      );
      assert.equal(
  ...
  describe("Summary and Aggregation", () => {
    test("should aggregate progress from multiple TaskTool agents", async () => {
      ...
      await taskTool1.reportProgress({
        status: "in_progress",
        progressPercent: 30,
      });
      ...
      await taskTool3.reportProgress({
        status: "completed",
        progressPercent: 100,
      });
      ...
      assert.equal(
        ...
        70,
        "Should calculate average progress (30+80+100)/3 = 70)",
      );
      assert.equal(
        summary.summary,
        "2 active, 1 done",
  ...
  describe("Error Handling Integration", () => {
    test("should handle TaskTool errors gracefully in ProgressTracker", async () => {
      ...
      assert.ok(
        result.error.includes("storage failed"),
    ...
    test("should work when ProgressTracker is not available", async () => {
      ...
      assert.equal(
        result.success,
        true,
        "Should succeed without progress tracker",
      );
  ...
  describe("Real-world Scenarios", () => {
    test("should handle complex multi-agent workflow", async () => {
      ...
      assert.equal(
        summary.statusCounts.needs_help,
        1,
        "Should have 1 agent needing help",
      );
      assert.equal(
        summary.helpRequests.length,
        1,
        "Should include help request",
      );
      ...
      assert.equal(
        activeAgents.length,
        3,
        "All agents should be active (none completed/failed)",
      );
