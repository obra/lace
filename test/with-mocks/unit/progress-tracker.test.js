...
describe("ProgressTracker", () => {
  ...
  describe("Initialization", () => {
    test("should create ProgressTracker instance", async () => {
      assert.ok(
        progressTracker instanceof Object,
        "ProgressTracker should be created",
      );
      assert.equal(
        typeof progressTracker.updateProgress,
        "function",
        "Should have updateProgress method",
      );
      assert.equal(
        typeof progressTracker.getProgress,
        "function",
        "Should have getProgress method",
      );
      assert.equal(
        typeof progressTracker.getProgressSummary,
        "function",
        "Should have getProgressSummary method",
      );
    });
    ...
    test("should use default options", async () => {
      ...
      assert.equal(
        defaultTracker.cleanupInterval,
        300000,
        "Should use default cleanup interval",
      );
      assert.equal(
        defaultTracker.maxAge,
        3600000,
        "Should use default max age",
      );
      assert.equal(
        defaultTracker.maxEntries,
        1000,
        "Should use default max entries",
      );
  ...
  describe("updateProgress", () => {
    ...
    test("should use current timestamp if not provided", async () => {
      ...
      assert.ok(
        stored.timestamp >= before && stored.timestamp <= after,
    ...
    test("should merge with existing progress data", async () => {
      ...
      assert.equal(
        ...
        75,
        "Should update progress percent",
      );
      assert.equal(
        stored.details,
        "Starting",
    ...
    test("should fail with invalid parameters", async () => {
      ...
      try {
        ...
        assert.ok(
          error.message.includes("Agent ID"),
          "Should mention agent ID requirement",
        );
      }
      ...
      try {
        ...
        assert.ok(
          ...
          "Should mention status requirement",
        );
      }
    });
    ...
    test("should trigger callbacks", async () => {
      ...
      assert.equal(
        callbackEvents[0].eventType,
        "progress_update",
        "Should be progress_update event",
      );
      assert.equal(
        callbackEvents[0].data.agentId,
        "agent-1",
        "Should include agent ID",
      );
      assert.ok(
        callbackEvents[0].data.progress,
  ...
  describe("getProgressSummary", () => {
    test("should return summary for no agents", async () => {
      ...
      assert.deepEqual(
        ...
        {},
        "Should have empty status counts",
      );
    ...
    test("should aggregate status counts", async () => {
      ...
      assert.equal(
        ...
        2,
        "Should count in_progress agents",
      );
      assert.equal(
        summary.statusCounts.completed,
        1,
        "Should count completed agents",
      );
      assert.equal(
        summary.statusCounts.failed,
        1,
        "Should count failed agents",
      );
      assert.equal(
        summary.overallProgress,
        67,
        "Should calculate average progress (30+70+100)/3 = 67)",
      );
    });
    ...
    test("should filter by specific agent IDs", async () => {
      ...
      assert.equal(
        summary.totalAgents,
        2,
        "Should only count specified agents",
      );
    ...
    test("should include help requests in summary", async () => {
      ...
      assert.equal(
        summary.helpRequests.length,
        1,
        "Should include help requests",
      );
      assert.equal(
        summary.helpRequests[0].agentId,
        "agent-1",
        "Should include agent ID",
      );
      assert.ok(
        summary.helpRequests[0].helpRequest,
    ...
    test("should generate concise summary text", async () => {
      ...
      assert.equal(
        summary.summary,
        "2 active, 1 done",
        "Should generate concise summary",
      );
    });
  });
  ...
  describe("removeProgress", () => {
    test("should remove progress for specific agent", async () => {
      ...
      assert.equal(
        progressTracker.getProgress("agent-1"),
        null,
        "Should remove agent-1 progress",
      );
      assert.ok(
        progressTracker.getProgress("agent-2"),
        "Should keep agent-2 progress",
      );
    });
  ...
  describe("clearAll", () => {
    test("should clear all progress data", async () => {
      ...
      assert.equal(
        progressTracker.getAllProgress().length,
        0,
      ...
      assert.equal(
        progressTracker.getProgress("agent-1"),
        null,
        "Should clear agent-1",
      );
      assert.equal(
        progressTracker.getProgress("agent-2"),
        null,
        "Should clear agent-2",
      );
    });
  ...
  describe("Callbacks", () => {
    test("should add and remove callbacks", async () => {
      ...
      assert.equal(
        progressTracker.callbacks.size,
        3,
        "Should have 3 callbacks (including test callback)",
      );
      ...
      assert.equal(
        progressTracker.callbacks.size,
        2,
        "Should have 2 callbacks after removal",
      );
    });
    ...
    test("should ignore non-function callbacks", async () => {
      ...
      assert.equal(
        progressTracker.callbacks.size,
        initialSize,
        "Should not add non-function callbacks",
      );
    });
  ...
  describe("Helper Methods", () => {
    ...
    test("should get active agents", async () => {
      ...
      assert.ok(
        activeIds.includes("agent-1"),
        "Should include in_progress agent",
      );
  ...
  describe("Cleanup and Memory Management", () => {
    test("should cleanup old entries", async () => {
      ...
      assert.equal(
        testTracker.getAllProgress().length,
        1,
    ...
    test("should respect max entries limit", async () => {
      ...
      for (let i = 1; i <= 7; i++) {
        await progressTracker.updateProgress(`agent-${i}`, {
          status: "working",
        });
      }
  ...
  describe("Destroy", () => {
    test("should clean up resources on destroy", async () => {
      ...
      assert.equal(
        progressTracker.callbacks.size,
        0,
        "Should clear all callbacks",
      );
      assert.equal(
        progressTracker.cleanupTimer,
        null,
        "Should clear cleanup timer",
      );
    });
  });
});
