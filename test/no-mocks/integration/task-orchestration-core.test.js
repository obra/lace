...
describe("Task Orchestration Core Integration", () => {
  ...
  describe("Inter-Agent Message Passing", () => {
    it("should send and receive messages between agents", async () => {
      ...
      expect(receiveResult.messages[0].content).toBe(
        "Parent task 50% complete",
      );
      ...
      const toolCalls = [
        {
          ...
          input: { status: "starting", details: "Task 1" },
        },
        {
          name: "task_reportProgress",
          input: { status: "processing", details: "Task 2" },
        },
        {
          name: "task_reportProgress",
          input: { status: "finishing", details: "Task 3" },
        },
      ];
  ...
  describe("Progress Tracking Integration", () => {
    ...
    it("should track agents needing help", async () => {
      ...
      expect(agentsNeedingHelp[0].helpRequest.helpNeeded).toContain(
  ...
  describe("Error Recovery and Retry Logic", () => {
    it("should retry failed operations with exponential backoff", async () => {
      ...
      toolRegistry.callTool = jest
        .fn()
        .mockImplementation(async (name, method, params) => {
          attemptCount++;
          if (attemptCount <= 2) {
      ...
      const toolCalls = [
        { name: "task_reportProgress", input: { status: "retrying" } },
      ];
    ...
    it("should not retry non-retriable errors", async () => {
      ...
      const toolCalls = [
        { name: "task_reportProgress", input: { status: "auth_test" } },
      ];
