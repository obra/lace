...
describe("Step 8: Code Syntax Highlighting Integration", () => {
  ...
  test("Message component handles mixed content with code blocks", () => {
    const messageProps = {
      type: "assistant" as const,
      content:
        "Here is an explanation followed by code:\n\nThis function adds two numbers:\n\n```javascript\nfunction add(a, b) {\n  return a + b;\n}\n```\n\nUse it like this: `add(2, 3)`",
    };
  ...
  test("syntax highlighting does not affect non-code messages", () => {
    const messageProps = {
      type: "assistant" as const,
      content:
        "This is just regular text without any code blocks. No highlighting should be applied here.",
    };
  ...
  test("syntax highlighting utility functions work correctly", async () => {
    const { detectCodeBlocks, highlightCode, processContentWithHighlighting } =
      await import("@/ui/utils/syntax-highlight.js");
