...
describe("Step 12: Streaming Text Support", () => {
  test("streaming message type displays with cursor indicator", () => {
    ...
    const prefixElement = messageChildren.find(
    ...
    const cursorElement = messageChildren.find(
      (child: any) => child?.props?.children === "▌",
    );
  ...
  test("streaming message displays partial content with typing cursor", () => {
    ...
    const contentElement = messageChildren.find(
      (child: any) => child.props?.children === "Hello wo",
    );
  ...
  test("StatusBar shows streaming indicator during text stream", () => {
    ...
    const streamingElement = fragmentChildren.find(
      (child: any) => child.props?.children === "Streaming...",
    );
  ...
  test("long streaming text updates without flickering", () => {
    const longText =
      "This is a very long streaming response that should update smoothly without causing flickering or performance issues. ".repeat(
        10,
      );
  ...
  test("streaming state transitions correctly from loading to streaming to complete", () => {
    ...
    const prefixElement = loadingChildren.find(
      (child: any) => child?.props?.children === "⠋ ",
    );
    expect(prefixElement).toBeTruthy();
    ...
    const hasCursor = streamingChildren.some(
      (child: any) => child.props?.children === "▌",
    );
    ...
    const hasNoCursor = !completeChildren.some(
      (child: any) => child.props?.children === "▌",
    );
  ...
  test("streaming respects navigation mode and highlighting", () => {
    ...
    const messageChildren = messageBox.props.children;
    const contentElement = messageChildren.find(
      (child: any) => child.props?.children === "Streaming content",
    );
