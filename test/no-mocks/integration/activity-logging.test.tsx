...
describe("Activity Logging Integration", () => {
  ...
  describe("ActivityLogger Basic Functionality", () => {
    ...
    test("should log user input events", async () => {
      await activityLogger.logEvent("user_input", "session-123", null, {
        content: "hello",
    ...
    test("should log agent response events", async () => {
      await activityLogger.logEvent("agent_response", "session-123", null, {
        content: "Hello world",
        tokens: 100,
        duration_ms: 250,
        timestamp: new Date().toISOString(),
    ...
    test("should log tool execution events", async () => {
      await activityLogger.logEvent("tool_execution", "session-123", null, {
        tool_name: "file_list",
        input: { path: "." },
    ...
    test("should handle streaming token events", async () => {
      ...
      for (let i = 0; i < tokens.length; i++) {
        await activityLogger.logEvent("streaming_token", "session-123", null, {
          token: tokens[i],
          timestamp: new Date().toISOString(),
        });
  ...
  describe("Activity Logger Error Handling", () => {
    test("should handle logging errors gracefully", async () => {
      ...
      await expect(
        activityLogger.logEvent("test_event", "session-123", null, {
          test: "data",
        }),
