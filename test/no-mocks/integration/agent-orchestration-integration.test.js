...
describe("Agent Orchestration Integration Tests", () => {
  ...
  describe("Error Recovery in Parallel Execution", () => {
    it("should handle mixed success/failure scenarios gracefully", async () => {
      ...
      let callCount = 0;
      toolRegistry.callTool = jest
        .fn()
        .mockImplementation(async (name, method, params, sessionId, agent) => {
          callCount++;
    ...
    it("should provide actionable error analysis", async () => {
      ...
      const originalReportProgress = toolRegistry
        .get("task")
        .reportProgress.bind(toolRegistry.get("task"));
      let callCount = 0;

      toolRegistry.get("task").reportProgress = jest
        .fn()
      ...
      expect(errorAnalysis.recommendations).toEqual(
        expect.arrayContaining([expect.stringContaining("tool-specific")]),
      );
  ...
  describe("Progress Tracking and Coordination", () => {
    ...
    it("should demonstrate request help functionality", async () => {
      ...
      const helpRequest = await taskTool.requestHelp({
        errorDescription:
          "Unable to process large dataset due to memory constraints",
        attemptedSolutions: [
  ...
  describe("End-to-End Orchestration Workflow", () => {
    it("should execute complete parallel workflow with all features", async () => {
      ...
      const failedResults = response.toolResults?.filter(
        (r) => !r.success && r.catastrophicFailure,
      );
    ...
    it("should demonstrate sequential fallback when parallel execution fails", async () => {
      ...
      const originalExecuteToolsInParallelWithRetry =
        orchestratorAgent.executeToolsInParallelWithRetry.bind(
          orchestratorAgent,
        );

      let callCount = 0;
      orchestratorAgent.executeToolsInParallelWithRetry = jest
        .fn()
        .mockImplementation(async (toolCalls, sessionId, reasoning) => {
          callCount++;
          ...
          if (callCount === 1) {
            const results = toolCalls.map((toolCall, index) => ({
              ...
              result:
                index < 2
                  ? { success: true, result: "Parallel success" }
                  : null,
          ...
          return originalExecuteToolsInParallelWithRetry(
            toolCalls,
            sessionId,
            reasoning,
          );
      ...
      // Restore original method
      orchestratorAgent.executeToolsInParallelWithRetry =
        originalExecuteToolsInParallelWithRetry;
    });
  });
});
