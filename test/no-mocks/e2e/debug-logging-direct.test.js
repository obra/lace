...
describe('Debug Logging Direct Tests', () => {
  ...
  test('DebugLogger should create log file and write messages', async () => {
    ...
    assert.ok(logContent.includes('[DEBUG] Test debug message'), 'Should contain debug message');
    assert.ok(logContent.includes('[INFO ] Test info message'), 'Should contain info message');
    assert.ok(logContent.includes('[WARN ] Test warning message'), 'Should contain warning message');
    assert.ok(logContent.includes('[ERROR] Test error message'), 'Should contain error message');
  });
  ...
  test('DebugLogger should respect log levels', async () => {
    ...
    assert.ok(!logContent.includes('[DEBUG]'), 'Should not contain debug messages');
    assert.ok(logContent.includes('[INFO ]'), 'Should contain info messages');
    assert.ok(logContent.includes('[WARN ]'), 'Should contain warning messages');
  ...
  test('LaceUI should initialize debugLogger correctly', async () => {
    ...
    assert.ok(logContent.includes('[INFO ] LaceUI debugLogger test message'), 
  ...
  test('Agent should receive debugLogger instance from LaceUI', async () => {
    ...
    assert.strictEqual(agent.debugLogger, laceUI.debugLogger, 
    ...
    assert.ok(logContent.includes('[INFO ] Agent debugLogger test message'),
  ...
  test('CLI options should be parsed correctly', async () => {
    ...
    const testCases = [
      {
        options: { logLevel: 'debug', logFile: testLogFile, logFileLevel: 'info' },
        expectedStderrLevel: 'debug',
        expectedFileLevel: 'info'
      },
      {
        options: { logLevel: 'off', logFile: testLogFile, logFileLevel: 'debug' },
    ...
    for (const testCase of testCases) {
      ...
      assert.strictEqual(debugLogger.stderrLevel, testCase.expectedStderrLevel,
        `stderr level should be ${testCase.expectedStderrLevel}`);
      assert.strictEqual(debugLogger.fileLevel, testCase.expectedFileLevel,
        `file level should be ${testCase.expectedFileLevel}`);
      assert.strictEqual(debugLogger.filePath, testLogFile,
