...
describe('StatusBar Component', () => {
  ...
  test('displays navigation hint in normal mode', () => {
    ...
    const navElement = fragmentChildren.find((child: any) => 
      child.type === Text && child.props.children === '↑/↓ to navigate, / to search'
    );
  ...
  test('displays model name with correct styling', () => {
    ...
    const modelFragment = children.find((child: any) => 
      child && Array.isArray(child.props?.children) &&
      child.props.children.some((subchild: any) => 
        subchild?.type === Text && subchild?.props?.children === 'claude-3.5-sonnet'
      )
    );
    
    expect(modelFragment).toBeTruthy();
    const modelElement = modelFragment.props.children.find((child: any) => 
  ...
  test('handles responsive layout for narrow terminals', () => {
    ...
    const modelFragment = children.find((child: any) => 
      child && Array.isArray(child.props?.children) &&
      child.props.children.some((subchild: any) => 
        subchild?.type === Text && subchild?.props?.children === 'claude-3.5'
      )
    );
