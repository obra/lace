...
import { basicCommands, agentCommands, toolManagementCommands, getAllCommands } from '../../../src/ui/commands/registry';
import type { CommandContext } from '../../../src/ui/commands/types';
...
describe('Command Registry', () => {
  ...
  beforeEach(() => {
    const mockCommandManager = {
      getHelpText: jest.fn(() => 'Available commands:\n  /help - Show help\n  /quit - Exit')
    };
  ...
  describe('agentCommands', () => {
    ...
    it('should handle memory command', async () => {
      ...
      expect(mockAgent.getConversationHistory).toHaveBeenCalledWith('test-session', 10);
  ...
  describe('toolManagementCommands', () => {
    it('should handle auto-approve command with argument', async () => {
      const autoApproveCmd = toolManagementCommands.find(cmd => cmd.name === 'auto-approve')!;
      ...
      expect(mockAgent.toolApproval.addAutoApprove).toHaveBeenCalledWith('file');
    });
    ...
    it('should handle deny command with argument', async () => {
      const denyCmd = toolManagementCommands.find(cmd => cmd.name === 'deny')!;
      ...
      expect(mockAgent.toolApproval.addDenyList).toHaveBeenCalledWith('dangerous-tool');
    });
  ...
  describe('getAllCommands', () => {
    it('should return all commands', () => {
      ...
      expect(allCommands.length).toBe(
        basicCommands.length + agentCommands.length + toolManagementCommands.length
      );
