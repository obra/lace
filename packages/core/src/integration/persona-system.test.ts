...
describe('Persona System Integration', () => {
  ...
  it('creates agent with persona and loads correct prompt configuration', async () => {
    ...
    expect(agentInfo.persona).toBe('coder');
  ...
  it('handles full task workflow with persona agents', async () => {
    ...
    const mockAgentCreator: AgentCreationCallback = vi
      ...
      .mockImplementation(async (persona, provider, model, task) => {
        expect(persona).toBe('_helper-agent');
    ...
    const task = await taskManager.createTask(
      {
        ...
        assignedTo: createNewAgentSpec('_helper-agent', 'anthropic', 'claude-3-sonnet'),
      },
      { actor: 'user' }
    );
  ...
  it('prompt manager generates different prompts for different personas', async () => {
    ...
    const codingPrompt = await promptManager.generateSystemPrompt('coder');
  ...
  it('loadPromptConfig integrates with persona system', async () => {
    ...
    const config2 = await loadPromptConfig({ persona: 'coder' });
    const config3 = await loadPromptConfig({ persona: '_helper-agent' });
  ...
  it('persona registry discovers all built-in personas', () => {
    ...
    expect(builtInPersonas.length).toBeGreaterThanOrEqual(4);
  });

  it('newagentspec parsing integrates correctly', () => {
    const specs = [
      'new:lace:anthropic/claude-3-sonnet',
      'new:coder:openai/gpt-4',
      'new:_helper-agent:ollama/llama2',
    ];
  ...
  it('agent configuration flows through correctly', () => {
    ...
    expect(info.persona).toBe('coder');
