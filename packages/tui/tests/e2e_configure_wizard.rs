...
fn send_request(transport: &AgentTransport, state: &mut AppState, out: Outbound) {
  match out {
    Outbound::JsonRpcRequest { id, method, params } => {
      transport
        .send_line(jsonrpc::encode_request(Value::String(id.clone()), &method, params))
        .unwrap();
...
fn e2e_configure_wizard_against_fake_agent() {
  ...
  while Instant::now() < deadline {
    ...
    match inbound {
      jsonrpc::InboundMessage::Response { id, result, error } => {
        ...
        if state.connection_id.as_deref() == Some("conn_1") && state.model_id.as_deref() == Some("gpt-test") {
          break;
        }
      }
      _ => {}
  ...
  assert_eq!(state.config_wizard.step, config_wizard::ConfigWizardStep::Done);
}
