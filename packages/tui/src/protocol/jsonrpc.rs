...
mod tests {
  ...
  #[test]
  fn parses_request() {
    let msg = parse_inbound(r#"{"jsonrpc":"2.0","id":"c_1","method":"initialize","params":{"a":1}}"#)
      .unwrap();
    assert_eq!(
      msg,
  ...
  fn parses_notification() {
    let msg = parse_inbound(r#"{"jsonrpc":"2.0","method":"session/update","params":{"type":"turn_end"}}"#)
      .unwrap();
    assert_eq!(
      msg,
  ...
  fn parses_response_error() {
    let msg = parse_inbound(r#"{"jsonrpc":"2.0","id":"c_2","error":{"code":0,"message":"nope"}}"#)
      .unwrap();
    assert_eq!(
      msg,
      InboundMessage::Response {
  ...
  fn encodes_request_round_trip() {
    let encoded = encode_request(json!("c_1"), "initialize", Some(json!({"protocolVersion":"1.0"})));
