...
pub fn jump_selected(state: &mut AppState) {
  let Some(item) = state.search.results.get(state.search.selected) else { return };
  match item.target {
...
pub fn jump_last_error(state: &mut AppState) {
  if let Some((idx, _)) = state
    ...
    .enumerate()
    .rev()
    .find(|(_, i)| matches!(i.kind, activity::ActivityKind::RpcError | activity::ActivityKind::Timeout))
  {
    state.focus = Focus::Activity;
...
fn recompute(state: &mut AppState) {
  ...
  for (idx, m) in state.messages.iter().enumerate() {
    ...
    state.search.results.push(SearchResult {
      ...
      target: SearchTarget::ChatScroll(chat_start_line_for_message_index(&state.messages, idx)),
    });
  }
...
mod tests {
  ...
  fn search_finds_chat_and_activity() {
    ...
    recompute(&mut state);
    assert!(state.search.results.iter().any(|r| r.label.contains("activity: error")));
  }
}
