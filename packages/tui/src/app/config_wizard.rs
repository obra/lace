...
pub fn next(state: &mut AppState) {
  let max = match state.config_wizard.step {
    ConfigWizardStep::SelectConnection => state.config_wizard.connections.len().saturating_sub(1),
...
pub fn handle_response(
  ...
  if let Some(err) = error_message {
    if is_method_not_found(err) {
      ...
      state.config_wizard.error_message = Some("configuration not supported by this agent".to_string());
      return Vec::new();
    }
  ...
  if let Some(Value::Object(obj)) = result {
    if let Some(Value::Array(arr)) = obj.get("connections") {
      for c in arr {
        ...
        let Some(connection_id) = cobj.get("connectionId").and_then(|v| v.as_str()) else { continue };
        let name = cobj.get("name").and_then(|v| v.as_str()).map(|s| s.to_string());
...
fn submit_connection(state: &mut AppState) -> Vec<Outbound> {
  let idx = state.config_wizard.selected.min(state.config_wizard.connections.len().saturating_sub(1));
  let Some(conn) = state.config_wizard.connections.get(idx) else { return Vec::new() };
  ...
  if let Some(Value::Object(obj)) = result {
    if let Some(Value::Array(arr)) = obj.get("providers") {
      for p in arr {
        ...
        let Some(provider_id) = pobj.get("providerId").and_then(|v| v.as_str()) else { continue };
...
fn submit_provider(state: &mut AppState) -> Vec<Outbound> {
  let idx = state.config_wizard.selected.min(state.config_wizard.providers.len().saturating_sub(1));
  let Some(p) = state.config_wizard.providers.get(idx) else { return Vec::new() };
  ...
  vec![Outbound::JsonRpcRequest {
    ...
    params: Some(json!({ "providerId": provider_id, "connection": { "name": "default", "config": {} } })),
  }]
}
...
fn on_credentials_start(state: &mut AppState, result: &Option<Value>) -> Vec<Outbound> {
  let connection_id = state.config_wizard.connection_id.clone().unwrap_or_default();
  let Some(Value::Object(obj)) = result else {
  ...
  state.config_wizard.credential_fields = fields
    ...
    .filter_map(|f| {
      ...
      let label = fobj.get("label").and_then(|v| v.as_str()).map(|s| s.to_string());
      let secret = fobj.get("secret").and_then(|v| v.as_bool()).unwrap_or(false);
      Some(CredentialField { name, label, secret })
    })
    .collect();
...
fn submit_credential_value(state: &mut AppState) -> Vec<Outbound> {
  ...
  let Some(field) = state.config_wizard.credential_fields.get(idx) else { return Vec::new() };
...
fn try_autofill_current_credential_field(state: &mut AppState) {
  let idx = state.config_wizard.credential_field_index;
  let Some(field) = state.config_wizard.credential_fields.get(idx) else { return };
  if !field.secret {
    return;
  }
...
fn submit_model(state: &mut AppState) -> Vec<Outbound> {
  let idx = state.config_wizard.selected.min(state.config_wizard.models.len().saturating_sub(1));
  let Some(model_id) = state.config_wizard.models.get(idx) else { return Vec::new() };
...
mod tests {
  ...
  fn open_sends_connections_list_request() {
    ...
    assert_eq!(state.config_wizard.step, ConfigWizardStep::LoadingConnections);
  }
