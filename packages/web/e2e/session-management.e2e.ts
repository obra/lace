...
test.describe('Session Management', () => {
  ...
  test('session URL persistence across page reloads', async ({ page, testEnv }) => {
    ...
    if (urlMatch) {
      ...
      const messageInputDisabled = await page.locator('[data-testid="message-input"]').getAttribute('disabled');
      const placeholder = await page.locator('[data-testid="message-input"]').getAttribute('placeholder');
  ...
  test('maintains session isolation between different workers', async ({ page, testEnv }) => {
    ...
    if (urlMatch) {
      ...
      try {
        ...
        await expect(chatInterface.getMessage(functionalTestMessage)).toBeVisible({ timeout: 10000 });
