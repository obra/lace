...
test.describe('Directory Browser E2E Tests', () => {
  test('should show directory browser when creating project', async ({ page }) => {
    await withIsolatedServer('directory-browser-', async (serverUrl, tempDir) => {
    ...
    test('should handle directory input validation', async ({ page }) => {
      await withIsolatedServer('directory-validation-', async (serverUrl, tempDir) => {
        // Navigate to the isolated server
        await page.goto(serverUrl);
    ...
    test('should work with valid directory paths', async ({ page }) => {
      await withIsolatedServer('directory-valid-paths-', async (serverUrl, tempDir) => {
        // Navigate to the isolated server
        await page.goto(serverUrl);
    ...
    test('should show directory browser dropdown when focused', async ({ page }) => {
      await withIsolatedServer('directory-dropdown-', async (serverUrl, tempDir) => {
        // Navigate to the isolated server
        await page.goto(serverUrl);
        ...
        const browserElements = page.locator(
          'text=/Browse:|Loading directories|No directories found/'
        );
    ...
    test('should handle keyboard navigation in directory field', async ({ page }) => {
      await withIsolatedServer('directory-keyboard-', async (serverUrl, tempDir) => {
        // Navigate to the isolated server
        await page.goto(serverUrl);
    ...
    test('should integrate with project creation workflow', async ({ page }) => {
      await withIsolatedServer('directory-integration-', async (serverUrl, tempDir) => {
        // Navigate to the isolated server
        await page.goto(serverUrl);
        ...
        // Use isolated server temp directory for valid project creation
        const projectPath = `${tempDir}/e2e-directory-test`;
