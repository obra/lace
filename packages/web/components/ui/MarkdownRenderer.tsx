  ...
  const components: Components = {
    ...
    code: ({ className, children, ...props }) => {
      ...
      if (!className || (typeof className === 'string' && !className.includes('language-'))) {
        return (
          <code
            className="bg-base-200 px-1 py-0.5 rounded text-sm font-mono text-base-content"
            {...props}
          >
            {children}
          </code>
    ...
    pre: ({ children, ...props }) => (
      <pre
        className="bg-base-200 rounded-lg p-3 overflow-x-auto text-sm border border-base-300"
        {...props}
      >
        {children}
      </pre>
    ...
    h1: ({ children }) => (
      <h1 className="text-xl font-bold mt-4 mb-3 text-base-content">{children}</h1>
    ),
    h2: ({ children }) => (
      <h2 className="text-lg font-semibold mt-4 mb-2 text-base-content">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-base font-semibold mt-4 mb-2 text-base-content">{children}</h3>
    ),
    strong: ({ children }) => (
    ...
    <div className={`bg-base-100 border border-base-300 rounded-lg p-4 ${className}`}>
      ...
      {shouldFold && !isExpanded && (
        <div className="text-center mt-3 pt-3 border-t border-base-300">
          <button onClick={toggleExpanded} className="text-xs text-primary hover:underline">
