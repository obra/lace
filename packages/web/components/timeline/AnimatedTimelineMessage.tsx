  ...
  if (entry.type === 'human') {
    return (
      ...
      >
        ...
        >
          <motion.div
            className="w-8 h-8 rounded-md bg-primary text-primary-content flex items-center justify-center text-sm font-medium"
          ...
          >
            ...
            >
              {entry.result?.content
                ?.map((block, i) => (block.type === 'text' ? block.text : ''))
                .join('')}
            </motion.div>
          </motion.div>
        </motion.div>
