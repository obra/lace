              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-base-content/70">
                    <strong className="text-base-content">{String(data.originalEventCount)}</strong> → <strong className="text-base-content">{String(data.compactedEvents.length)}</strong> events
                  </span>
                  {preservedMessages !== undefined && preservedMessages > 0 && (
                    <span className="text-base-content/70">
                      <strong className="text-base-content">{String(preservedMessages)}</strong> user messages preserved
                    </span>
                  )}
                  {recentEvents !== undefined && recentEvents > 0 && (
                    <span className="text-base-content/70">
                      <strong className="text-base-content">{String(recentEvents)}</strong> recent events
                    </span>
                  )}
                </div>
