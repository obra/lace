...
console.log(`Starting Lace in production mode...`);
...
// Change to the standalone root directory (like Next.js does) for proper module resolution
const standaloneRoot = path.resolve(__dirname, '../..');
process.chdir(standaloneRoot);
const webDir = path.join(standaloneRoot, 'packages/web');
console.log(`Current working directory: ${process.cwd()}`);
console.log(`Web directory: ${webDir}`);

// Next.js configuration from the standalone build
const nextConfig = {
  env: {},
...
// Set the Next.js private config environment variable
