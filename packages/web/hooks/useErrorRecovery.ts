...
export function useErrorRecovery() {
  ...
  const retryAgentOperation = useCallback(
    async (threadId: string, errorType: ErrorType): Promise<boolean> => {
  ...
  const retryToolOperation = useCallback(
    async (threadId: string, toolCallId: string, toolName: string): Promise<boolean> => {
  ...
  const canRetry = useCallback(
    (errorId: string, errorType: ErrorType): boolean => {
