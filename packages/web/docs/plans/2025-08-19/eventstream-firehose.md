...
Currently, each React component that needs real-time events creates its own
EventSource connection via the `useEventStream` hook. This results in:
...
Replace the current per-component EventSource pattern with a **firehose
singleton**: one EventSource connection that receives ALL events, with pure
client-side filtering. Components subscribe to the events they need via simple
boolean filters.

## Benefits of Firehose Approach

1. **Simple server**: No query parameters, no filtering logic - just send
   everything
2. **No resubscription**: Connection never changes, stays open regardless of
   component needs
...
2. **Server manager**: `packages/web/lib/event-stream-manager.ts` - Connection
   management
3. **Client hook**: `packages/web/hooks/useEventStream.ts` - React integration
4. **Types**: `packages/web/types/core.ts` and
...
- **Location**: Test files go next to source files (e.g., `singleton.ts` →
  `singleton.test.ts`)
...
**Objective:** Remove all query parameter handling since we're using a firehose
approach.
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller: ReadableStreamDefaultController<Uint8Array>) {
        ...
        const cleanup = () => {
          if (process.env.NODE_ENV === 'development') {
            console.log(
              `[EVENT_STREAM_SERVER] Cleaning up firehose connection ${connectionId.substring(0, 8)}`
            );
          }
      ...
      cancel(connectionId?: string) {
        ...
        if (connectionId && typeof connectionId === 'string') {
          if (process.env.NODE_ENV === 'development') {
            console.log(
              `[EVENT_STREAM_SERVER] Firehose stream cancelled for connection ${connectionId.substring(0, 8)}`
            );
          }
          manager.removeConnection(connectionId);
        }
...
**Objective:** Remove server-side filtering logic since client handles all
filtering.
...
**That's it!** The firehose approach eliminates all the complex server-side
filtering logic.
...
**Objective:** Identify all places where the old `useEventStream` is currently
used so we can verify they work with the new implementation.
...
**Documentation:** Create a migration verification checklist:
...
1. **Removed**: `sendCount`, `close`, `autoReconnect`, `reconnectInterval`
   (handled internally)
...
5. **Improved**: No resubscription needed when filters change - client-side
...
- ✅ Only one EventSource connection regardless of number of subscribed
  components
...
1. The implementation is a complete replacement, so rollback means reverting the
   entire changeset
...
This firehose implementation is simpler, more reliable, and perfectly suited for
our single-user system architecture.
...
5. ⏭️ **Add Error Handling and Reconnection** - Skipped (YAGNI - basic error
   handling sufficient)
6. ✅ **Create React Hook Integration** - Complete
7. ✅ **Simplify Server-Side Implementation** - Complete
8. ✅ **Update Server-Side Connection Management** - Complete (already optimal
   for firehose)
9. ✅ **Find All useEventStream Usage in Codebase** - Complete (full
...
12. ⏭️ **Performance Monitoring and Metrics** - Skipped (YAGNI - basic stats
    sufficient)
...
- **Reliability**: No missed events during filter changes or component
  remounting
...
The EventStream Firehose Singleton is production-ready and provides a
significant architectural improvement over the previous per-component
EventSource pattern. The implementation follows all specified requirements:
...
The firehose pattern is perfectly suited for our single-user architecture and
eliminates the complexity of multiple EventSource connections while maintaining
