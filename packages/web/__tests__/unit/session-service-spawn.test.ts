...
describe('SessionService.spawnAgent Method', () => {
  ...
  it('should spawn agent via service and make it retrievable', async () => {
    ...
    expect(agent.name).toBe('Service Agent');
    expect(agent.provider).toBe('anthropic');
    expect(agent.status).toBe('idle');
    
    // Verify agent is retrievable via service
    const retrievedAgent = sessionService.getAgent(agent.threadId as ThreadId);
  ...
  it('should handle agent retrieval after spawning', async () => {
    ...
    const immediateAgent = sessionService.getAgent(agentThreadId);
    ...
    const _laterAgent = sessionService.getAgent(agentThreadId);
  ...
  it('should spawn multiple agents and keep them separate', async () => {
    ...
    const retrievedAgent1 = sessionService.getAgent(agent1.threadId as ThreadId);
    const retrievedAgent2 = sessionService.getAgent(agent2.threadId as ThreadId);
  ...
  it('should handle agent spawning in reconstructed session', async () => {
    ...
    expect(agent.threadId).toMatch(new RegExp(`^${sessionId}\\.\\d+$`));
    expect(agent.name).toBe('Reconstructed Agent');
  ...
  it('should handle thread persistence for spawned agents', async () => {
    ...
    const retrievedAgent = sessionService.getAgent(agentThreadId);
    expect(retrievedAgent).toBeDefined();
  ...
  it('should handle caching issues between service and ThreadManager', async () => {
    ...
    // Verify agent is retrievable via service
    const serviceAgent = sessionService.getAgent(agentThreadId);
    ...
    const event = freshThreadManager.addEvent(agentThreadId, 'USER_MESSAGE', 'Hello from fresh manager');
    ...
    if (serviceAgent) {
      ...
      const serviceThreadManager = (serviceAgent as { _threadManager: { getEvents: (id: ThreadId) => unknown[] } })._threadManager;
      ...
      // Find our specific event among the events (agent may add startup events)
      const ourEvent = eventsFromService.find((e: unknown) => (e as { id: string }).id === event.id);
      expect(ourEvent).toBeDefined();
  ...
  it('should handle error when spawning agent in non-existent session', async () => {
    ...
    await expect(
      sessionService.spawnAgent(
        'non-existent-session' as ThreadId,
        'Orphan Agent',
        'anthropic',
        'claude-3-haiku-20240307'
      )
