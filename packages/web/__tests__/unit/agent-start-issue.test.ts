...
describe('Agent Start Issue', () => {
  ...
  it('should reproduce the exact E2E test scenario', async () => {
    ...
    const threadManager = (sessionAgent as unknown as { _threadManager: ThreadManager })._threadManager;
    ...
    const _agentThreadManager = (agent as unknown as { _threadManager: ThreadManager })._threadManager;
  ...
  it('should check the exact agent creation flow', () => {
    ...
    const sessionAgent = session.getAgent(session.getId());
    const sessionThreadManager = (sessionAgent as unknown as { _threadManager: ThreadManager })._threadManager;
    ...
    const _event = sessionThreadManager.addEvent(delegateThreadId, 'SYSTEM_PROMPT', 'Starting agent...');
    expect(_event).toBeDefined();
  });
  ...
  it('should test the actual Agent constructor with delegate thread', async () => {
    ...
    const delegateAgent = new Agent({
      ...
      tools: (toolExecutor as unknown as { getAllTools: () => unknown[] }).getAllTools(),
    });
    
    expect(delegateAgent).toBeDefined();
    
    // Check if the agent can find its own thread
    const agentThreadManager = (delegateAgent as unknown as { _threadManager: ThreadManager })._threadManager;
