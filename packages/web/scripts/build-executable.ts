...
function parseArgs(): BuildOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--help':
        console.log(`
Usage: npx tsx build-executable.ts [options]
...
Examples:
  npx tsx build-executable.ts
  npx tsx build-executable.ts --target bun-linux-x64 --name lace-linux
`);
        process.exit(0);
    }
  }
...
async function buildSimpleExecutable(options: BuildOptions = {}) {
  ...
  if (!existsSync(nextBuildPath)) {
    throw new Error(`Next.js build not found at ${nextBuildPath}. Run: bun run build`);
  }
  ...
  if (!existsSync('server-custom.ts')) {
  ...
  // Use the executable-bundle.ts directly - Bun will bundle the ZIP file
  const execSourcePath = 'scripts/executable-bundle.ts';
