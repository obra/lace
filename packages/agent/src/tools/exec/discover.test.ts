// ABOUTME: Tests for discoverExecTools — scans a dir, builds ExecToolAdapters
...
describe('discoverExecTools', () => {
  it('returns [] for a missing directory', async () => {
    const result = await discoverExecTools('/nonexistent/path/that/does/not/exist');
    expect(result).toEqual([]);
  });

  it('builds an ExecToolAdapter for echo-tool.sh in the main fixtures dir', async () => {
    const result = await discoverExecTools(FIX);
  ...
  it('skips a schema-invalid binary without throwing, still returns valid ones', async () => {
    // DISCOVER_FIX has bad-schema-tool.sh (invalid schema) and valid-tool.sh
    const result = await discoverExecTools(DISCOVER_FIX);
