...
**Use this tool whenever you start a background job.** \`delegate(prompt=...)\` returns immediately with a \`jobId\`; \`bash(background=true)\` does the same. Subscribe with \`job_notify(jobId)\` to be woken on terminal (and optionally progress) transitions, then return to the user. It is the cheap, async path. \`job_output\` is a snapshot — use it to inspect a job's output on demand, not to wait on it. Subscribe, return to the user, do something else — lace will deliver a \`<notification kind="job-completed"|"job-failed"|"job-cancelled"|"job-progress" job-id="...">\` block on your next turn when the job transitions.
