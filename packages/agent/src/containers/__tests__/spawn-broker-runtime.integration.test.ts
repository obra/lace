...
describe.skipIf(!DOCKER_AVAILABLE || BLOCKED_ON_CANONICAL_ID_BUG)(
  'SpawnBrokerContainerRuntime (real docker)',
  () => {
    let dir: string;
    let scratch: string;
    ...
    afterAll(() => {
      // DockerContainerRuntime may store under a lace-prefixed name (resolveContainerName),
      // so remove both the catalog name and its lace-prefixed form.
      for (const name of createdNames) {
        forceRemove(name);
        forceRemove(`lace-${name}`);
      }
    });
