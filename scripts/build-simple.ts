...
function loadNFTTrace(traceFilePath: string): NFTTraceData | null {
  try {
    if (!existsSync(traceFilePath)) {
      console.warn(`⚠️  NFT trace file not found at ${traceFilePath}`);
      return null;
    }
    ...
    console.log(`📋 Loaded NFT trace from ${traceFilePath}`);
    console.log(`   Traced files: ${traceData.tracedFiles.length}`);
    console.log(
      `   Has required packages: ${traceData.summary.hasIsDocker && traceData.summary.hasOpen ? '✅' : '❌'}`
    );

    return traceData;
  } catch (error) {
    console.warn(`⚠️  Failed to load NFT trace: ${error}`);
    return null;
  }
}
...
async function copyTracedDependencies(
  ...
  console.log('📦 Copying traced dependencies...');
  ...
  for (const pattern of packagePatterns) {
    ...
    try {
      if (existsSync(sourcePath)) {
        ...
        console.warn(`   ⚠️  Package not found: ${packageName} at ${sourcePath}`);
      }
    } catch (error) {
...
async function copyProductionDependencies(targetNodeModules: string): Promise<void> {
  ...
  try {
    ...
    if (existsSync(tempNodeModules)) {
      ...
      console.log(`   📦 Size: ${(stats.size / 1024 / 1024).toFixed(1)}MB (estimated)`);
    }
  } catch (error) {
